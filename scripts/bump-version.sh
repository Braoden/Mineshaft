...
# =============================================================================
# VERSION BUMP SCRIPT FOR MINESHAFT
# =============================================================================
#
# This script handles version bumping for Mineshaft releases.
...
usage() {
    ...
    echo ""
    echo "Bump version across all Mineshaft components."
    echo ""
    echo "Arguments:"
    ...
    echo "  --install        Rebuild and install ms binary to GOPATH/bin"
    echo ""
    ...
    echo "  $0 0.2.0 --install              # Update versions and rebuild/install ms"
...
main() {
    ...
    if [ "$AUTO_INSTALL" = true ]; then
        echo "Rebuilding and installing ms..."
        ...
        echo -e "${GREEN}✓ ms installed${NC}"

        # Verify - check both GOPATH/bin and ~/.local/bin (Makefile default)
        INSTALL_DIR="${HOME}/.local/bin"
        if [ -x "$INSTALL_DIR/ms" ]; then
            INSTALLED_VERSION=$("$INSTALL_DIR/ms" version 2>&1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
        else
            INSTALLED_VERSION=$(ms version 2>&1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
        fi

        if [ "$INSTALLED_VERSION" = "$NEW_VERSION" ]; then
            echo -e "${GREEN}✓ Verified: ms version $INSTALLED_VERSION${NC}"
        else
            echo -e "${YELLOW}⚠ ms reports $INSTALLED_VERSION (expected $NEW_VERSION)${NC}"
        fi
        echo ""
    fi
...
- ms CLI: $CURRENT_VERSION → $NEW_VERSION
...
Generated by scripts/bump-version.sh"
        ...
        if [ "$AUTO_PUSH" = true ]; then
            ...
            echo "Monitor: https://github.com/steveyegge/mineshaft/actions"
