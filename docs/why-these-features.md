...
> Mineshaft's architecture explained through enterprise AI challenges
...
Mineshaft treats work as structured data. Every action is recorded. Every agent
...
**The solution:** Every Mineshaft agent has a distinct identity. Every action is
attributed:

```
Git commits:    mineshaft/miners/toast <owner@example.com>
Beads records:  created_by: mineshaft/crew/joe
Event logs:     actor: mineshaft/miners/nux
```
...
bd audit --actor=mineshaft/miners/toast

# Success rate on Go projects
bd stats --actor=mineshaft/miners/toast --tag=go
```
...
bd skills mineshaft/miners/toast
...
> **Status: Planned** — Federation via the Highway Operations Protocol (HOP) is designed but not yet implemented. Mineshaft currently operates as a single-town system.
...
```json
{
  "validated_by": "mineshaft/refinery",
...
Mineshaft is a developer tool - like an IDE, but for AI orchestration. However,
...
Mineshaft is built to answer the questions enterprises will ask as AI agents
