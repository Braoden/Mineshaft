...
|---|-----------|----------|-------------|-------------------|
| I-1 | Cycles in blocking deps MUST prevent minecart creation | data | **high** | no |
| I-2 | Beads with no valid rig MUST prevent minecart creation | data | **high** | no |
...
| I-7 | Daemon MUST NOT feed issues from `staged:*` minecarts | safety | **high** | no |
| I-8 | `--launch` on `staged_warnings` MUST require `--force` | safety | medium | no |
| I-9 | Re-staging a minecart MUST NOT create duplicates | data | medium | no |
...
| F-05 | Minecart ID passed to stage that is already `open` | medium | status check | re-analyze and warn | no |
...
| F-09 | Stale minecart: re-stage finds beads deleted since first stage | low | bd show fails | error per-bead | no |
| F-10 | Concurrent staging of same beads by two users | low | no detection | last write wins (acceptable) | no |
| F-11 | Partial dep-add failure during minecart creation | medium | per-dep error check | partial tracking (existing pattern) | partial |
...
| # | Failure | Likelihood | Detection | Recovery | Covered? |
|---|---------|-----------|-----------|----------|----------|
| F-21 | Not inside a workspace (no overseer/rig/) | medium | workspace.FindFromCwd | clear error | no |
...
### Unit tests (package `cmd`, `minecart`)
...
| U-33 | JSON output: minecart_id empty when errors found | `renderJSON(result)` | -- | I-3, I-10 | P1 |
...
| IT-04 | Stage minecart: reads tracked beads from existing minecart | US-001 | F-05 | -- | P1 |
| IT-05 | Stage with cycle: refuses to create minecart | US-002 | F-07 | I-1, I-3 | P0 |
| IT-06 | Stage with no valid rig: refuses to create minecart | US-002 | F-23 | I-2, I-3 | P0 |
| IT-07 | Stage with errors: exit code non-zero, no bd create called | US-002 | -- | I-3 | P0 |
| IT-08 | Stage with parked rig: creates minecart as staged_warnings | US-003 | F-19 | -- | P1 |
| IT-09 | Stage epic with unreachable task (not in descendant tree): warns, creates staged_warnings | US-003 | F-08 | -- | P1 |
| IT-10 | Stage clean: creates minecart as staged_ready | US-007 | -- | -- | P0 |
| IT-11 | Stage minecart: tracks all slingable beads via deps | US-007 | F-11 | -- | P0 |
| IT-12 | Stage minecart: description includes wave count + timestamp | US-007 | -- | -- | P2 |
| IT-13 | Re-stage: updates status, no duplicate minecart | US-007 | F-09 | I-9 | P1 |
...
| IT-41 | Stage clean: minecart ID printed to stdout | US-007 | -- | -- | P1 |
...
### Integration tests (real Dolt store, package `minecart`)
...
| DS-07 | Event-driven path skips staged_ready minecart (`CheckMinecartsForIssue` → `feedNextReadyIssue`) | US-007 | -- | I-7 | P0 |
| DS-08 | Event-driven path skips staged_warnings minecart | US-007 | -- | I-7 | P0 |
| DS-09 | Stranded scan path excludes staged minecarts (`findStrandedMinecarts` queries `--status=open`) | US-007 | -- | I-7 | P0 |
| DS-10 | Daemon feeds minecart after status transitions from staged_ready to open | US-008 | -- | I-7 | P1 |
...
| SN-03 | Launch output: minecart ID, wave summary, miner list, hints | US-009 | P2 |
...
### Property tests (package `cmd` or `minecart`)
...
**Compound Value:** Every new minecart test (stage, launch, future milestones) reuses this. The graph definition IS the test documentation.
...
|---------|---------------------|------------|
| Sleep-based sync | Daemon integration tests (DS-09, DS-10) wait for minecart status transitions; temptation to add delays | Use completion channels or check bd state, never `time.Sleep` in tests |
...
7. **Implement daemon staged-minecart guard** -- add staged-status check in `CheckMinecartsForIssue` (event-driven path, `operations.go`). Stranded scan path (`minecart.go:1231`) is already safe via `--status=open` query. Update `ensureKnownMinecartStatus` and `validateMinecartStatusTransition` for staged statuses.
...
| 4 | Minecart: reads tracked beads via dep list | IT-04 |
...
| 3 | Errors: no minecart, no status changes | IT-07, U-27 |
