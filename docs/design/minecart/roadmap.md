...
→ minecart closes when root epic closes
```
...
land on the target branch.** This is NOT a minecart problem — it's a
sling→done→refinery pipeline reliability problem. The minecart system layers
on top of this pipeline.

### Critical failure points (independent of minecarts)

| # | Failure | Where | Severity | Recovery |
|---|---------|-------|----------|----------|
| 1 | ~~Dolt branch merge fails~~ | ~~`done.go`~~ | Resolved | Eliminated by all-on-main architecture (no per-miner Dolt branches). |
...
These failures affect ALL miner work, not just minecart-tracked work.
Fixing them benefits the entire system.

### Minecart-specific failure points
...
### Milestone 1: Pipeline reliability (independent of minecarts)

**Goal:** Fix the sling→done→refinery pipeline failures that cause
"tasks don't land" complaints.

This is the highest-impact work for user-reported problems. Minecarts
...
|---|---------|-------------|------------|
| 1a | ~~Dolt branch merge fails~~ | Resolved — all-on-main eliminates per-miner Dolt branches. | N/A |
| 1b | ~~Stranded MR beads on Dolt branches~~ | Resolved — no per-miner Dolt branches to strand on. | N/A |
...
| 1d | Merge conflicts block indefinitely | Track conflict task age. If unresolved after N hours, escalate to Overseer/owner with the specific conflict details. | Low |
...
1. **Sub-epic completion trigger**: When the minecart's epic status
...
3. **Minecart awareness**: The minecart stays open while the review runs.
   The review miner's completion triggers the next sub-epic (if the
...
**Goal:** Pluggable dispatch strategies and coordinator miners.
...
common case. The coordinator miner is for complex epics where
...
MinecartManager's mechanical feeding handles the happy path; the judgment
...
with parent-child deps, tasks with blocks deps). For the staged minecart
workflow, it needs:
...
2. **Next:** Start Milestone 1 (pipeline reliability) and/or Milestone 2
   ...
   staged minecart UX. These can run in parallel.
