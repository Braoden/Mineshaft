...
their branch and open (or confirm) a GitHub PR before running `ms done`.
...
commands into the workflow a miner actually sees at `ms prime` time.

## Install

```bash
# Role directive (rig-scoped)
mkdir -p ~/ms/<rig>/directives
cp miner.md ~/ms/<rig>/directives/miner.md

# Formula overlay (rig-scoped)
mkdir -p ~/ms/<rig>/formula-overlays
cp mol-miner-work.toml ~/ms/<rig>/formula-overlays/mol-miner-work.toml
```
...
ms formula overlay show mol-miner-work --rig <rig>

# See the directive text that will be injected at prime time
ms directive show miner --rig <rig>
...
- Tells the miner to push and open a PR before `ms done`
...
- Modify `ms done` behavior (no Go changes)
...
labels, CODEOWNERS enforcement, CI checks before `ms done`), copy this harness
and adapt it. The point is a starting template, not a drop-in product.

## Fixing an existing PR (gh#3602)

By default `ms sling` creates a fresh `miner/<name>/<bead>@<ts>` branch from
...
ms sling <bead> <rig> --branch miner/example/gh-1234@abcdef

# Resume by PR number (resolves the head ref via `gh pr view`)
ms sling <bead> <rig> --pr 1234
```

The miner's worktree HEAD will land on the named branch, so its commits
extend the existing PR's history and `ms done` pushes back to the same ref.
