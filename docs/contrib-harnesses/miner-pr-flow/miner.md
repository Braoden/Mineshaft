## PR-Flow Miner Policy

> **Rig Policy — overrides formula instructions where they conflict.**

This rig uses a **miner → GitHub PR → human review** flow. After pushing
...
`mol-miner-work`. `ms done` is still the completion signal — but in this
rig, a visible PR is the gating artifact for review.

### Required steps after implementation

1. Push the branch explicitly (do not rely on `ms done` to push):
   ```bash
...
3. If no PR exists, create one against the base branch:
   ...
   ```
4. Only then run `ms done`.

### Do NOT

- Run `ms done` without a PR open — the review loop breaks.
...
```bash
ms mail send <rig>/witness -s "HELP: gh pr create failed" -m "Branch: $(git branch --show-current)
Error: <paste>
