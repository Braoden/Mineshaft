...
- Dolt 2.0.3 fixes an SSH child-process leak from `CALL dolt_fetch` against SSH remotes. This reduces risk for `ms dolt` remote flows; no Mineshaft workaround is needed.
...
- `ms dolt status`: server was running on port `3307`, query latency `0s`, `4 / 1000` connections, with one pre-existing orphan database (`testrig`) reported for cleanup.
- `ms scheduler status`: scheduler active, 3 scheduled beads, 1 ready, 3 active miners, and 9 free slots of 25.
- `go test ./internal/deps ./internal/doctor ./internal/testutil`: passed.
- Focused Dolt command tests under `./internal/cmd`: passed.
- `go build ./cmd/ms`: passed.
...
Non-blocking note: an intentionally broad `go test ./internal/cmd -run 'TestDolt|TestInstall.*Dolt|Test.*Dolt'` selection also matched `TestSlingSetsDoltAutoCommitOff` and failed because fixture bead `ms-test456` was absent. A focused Dolt command test selection passed.
