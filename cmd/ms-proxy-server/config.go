...
// ProxyConfig is the configuration file schema for ms-proxy-server.
// It is loaded from JSON and merged with CLI flags (flags take precedence).
// The default location is ~/ms/.runtime/proxy/config.json.
type ProxyConfig struct {
	...
	// Defaults to ~/ms/.runtime/ca if empty.
	CADir string `json:"ca_dir"`

	// TownRoot is the Mineshaft root directory (e.g. ~/ms).
	// Defaults to $MS_TOWN or ~/ms if empty.
	TownRoot string `json:"town_root"`

	// AllowedCommands is the list of binary names miners may execute (e.g. ["ms","bd"]).
