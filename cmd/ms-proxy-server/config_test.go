...
func TestLoadConfig(t *testing.T) {
	t.Run("parses all fields correctly", func(t *testing.T) {
		cfg := ProxyConfig{
			...
			CADir:           "/tmp/ca",
			TownRoot:        "/tmp/ms",
			AllowedCommands: []string{"ms", "bd"},
			AllowedSubcommands: map[string][]string{
				"ms": {"prime", "hook"},
				"bd": {"create", "show"},
			},
			ExtraSANIPs:   []string{"170.170.170.170", "10.8.0.1"},
			ExtraSANHosts: []string{"proxy.mycompany.com", "ms-proxy.local"},
		}
		...
		assert.Equal(t, "/tmp/ms", got.TownRoot)
		assert.Equal(t, []string{"ms", "bd"}, got.AllowedCommands)
		assert.Equal(t, []string{"prime", "hook"}, got.AllowedSubcommands["ms"])
		...
		assert.Equal(t, []string{"proxy.mycompany.com", "ms-proxy.local"}, got.ExtraSANHosts)
	})
}
...
func TestParseAllowedSubcmds(t *testing.T) {
	...
	t.Run("empty subcommand list skipped", func(t *testing.T) {
		// "ms:" has a colon but no subcommands after it — entry is skipped.
		result := parseAllowedSubcmds("ms:")
	...
	t.Run("missing semicolon between entries treats second group as subcommand", func(t *testing.T) {
		// Without a semicolon, "hookbd:create" is parsed as a single subcommand of ms.
		result := parseAllowedSubcmds("ms:prime,hookbd:create")
		assert.Equal(t, []string{"prime", "hookbd:create"}, result["ms"])
	...
	t.Run("duplicate subcommands are preserved", func(t *testing.T) {
		...
		result := parseAllowedSubcmds("ms:prime,prime,hook")
		assert.Equal(t, []string{"prime", "prime", "hook"}, result["ms"])
	})

	t.Run("single entry without semicolons parses correctly", func(t *testing.T) {
		result := parseAllowedSubcmds("ms:prime,hook,done")
		assert.Equal(t, []string{"prime", "hook", "done"}, result["ms"])
	})

	t.Run("multiple entries parse correctly", func(t *testing.T) {
		result := parseAllowedSubcmds("ms:prime,hook;bd:create,show")
		assert.Equal(t, []string{"prime", "hook"}, result["ms"])
		assert.Equal(t, []string{"create", "show"}, result["bd"])
	})

	t.Run("extra semicolons and empty parts are skipped", func(t *testing.T) {
		result := parseAllowedSubcmds(";ms:prime;;bd:create;")
		assert.Equal(t, []string{"prime"}, result["ms"])
		assert.Equal(t, []string{"create"}, result["bd"])
	})

	t.Run("whitespace is trimmed from cmd and subcommands", func(t *testing.T) {
		result := parseAllowedSubcmds(" ms : prime , hook ; bd : create ")
		assert.Equal(t, []string{"prime", "hook"}, result["ms"])
		assert.Equal(t, []string{"create"}, result["bd"])
	})
}
...
func TestBuildAllowedSubcmds(t *testing.T) {
	...
	t.Run("single entry serialises correctly", func(t *testing.T) {
		...
		assert.Equal(t, "ms:prime,hook", result)
	})
...
func TestParseAllowedSubcmdsRoundTrip(t *testing.T) {
	...
	t.Run("multi-entry: map content preserved after round-trip", func(t *testing.T) {
		...
		original := "ms:prime,hook,done,mail,nudge;bd:create,update,close,show,list"
