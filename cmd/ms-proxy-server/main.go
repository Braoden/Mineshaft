// ms-proxy-server is the mTLS proxy server for sandboxed miner execution.
// It runs on the host and allows containers to call ms/bd and access git repos
...
// defaultAllowedSubcmds lists the safe subcommands for ms and bd.
// Dangerous subcommands (e.g. ms miner, ms rig, ms admin, ms nuke) are excluded.
...
func main() {
	var (
		configFile     = flag.String("config", "", "path to config file (default: ~/ms/.runtime/proxy/config.json)")
		...
		caDir          = flag.String("ca-dir", "", "directory for CA cert/key (default: ~/ms/.runtime/ca)")
		allowedCmds    = flag.String("allowed-cmds", "ms,bd", "comma-separated list of allowed commands")
		...
		townRoot = flag.String("town-root", "", "Mineshaft root directory (default: $MS_TOWN or ~/ms)")
	)
	flag.Parse()
	...
	cfgPath := *configFile
	if cfgPath == "" {
		cfgPath = filepath.Join(home, "ms", ".runtime", "proxy", "config.json")
	}
	...
	if *caDir == "" {
		*caDir = filepath.Join(home, "ms", ".runtime", "ca")
	}

	if *townRoot == "" {
		if v := os.Getenv("MS_TOWN"); v != "" {
			*townRoot = v
		} else {
...
// discoverAllowedSubcmds calls "ms proxy-subcmds" to auto-discover the allowed
...
func discoverAllowedSubcmds() string {
	cmd := exec.Command("ms", "proxy-subcmds")
	util.SetDetachedProcessGroup(cmd)
	out, err := cmd.Output()
	if err != nil {
		slog.Debug("ms proxy-subcmds discovery failed, using built-in default", "err", err)
...
// "ms:prime,hook,done;bd:create,update,close" into a map of command → subcommand set.
