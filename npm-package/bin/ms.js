...
function getBinaryPath() {
  ...
  if (!fs.existsSync(binaryPath)) {
    console.error(`Error: ms binary not found at ${binaryPath}`);
...
function main() {
  ...
  // Spawn the native ms binary with all command-line arguments
  ...
  child.on('error', (err) => {
    console.error(`Error executing ms binary: ${err.message}`);
    process.exit(1);
  });
