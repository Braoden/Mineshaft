...
description = "Context-aware stuck/crashed agent detection and restart for miners and supervisors"
version = 1
...
Detects stuck or crashed miners and supervisors by inspecting tmux session context
...
Gather all miners and the supervisor session. We check crashed sessions
(`session-dead`, work on hook) and confirmed zombie sessions (`agent-dead`).
`agent-hung` is observe-only for miners.
...
# CRITICAL: We need both the rig name (for filesystem paths like $TOWN_ROOT/$RIG/miners/)
...
## Step 2: Check miner health

For each rig, enumerate miners and check their session status.
A miner is a concern if:
...
## Step 3: Check supervisor health

The supervisor session is `hq-supervisor`. Check heartbeat staleness from the JSON
`timestamp` field in `supervisor/heartbeat.json` (fall back to file mtime only if
the timestamp is missing or malformed). A live Supervisor with no `in_progress` work
...
if ! tmux has-session -t "$SUPERVISOR_SESSION" 2>/dev/null; then
  echo "  CRASHED: Supervisor session is dead"
  ...
  if [ -z "$SUPERVISOR_COMM" ]; then
    echo "  ZOMBIE: Supervisor process dead (pid=$SUPERVISOR_PID), session alive"
    SUPERVISOR_ISSUE="zombie"
  else
    echo "  Process alive: pid=$SUPERVISOR_PID comm=$SUPERVISOR_COMM"
  ...
  if [ -z "$SUPERVISOR_ISSUE" ] && [ -f "$HEARTBEAT_FILE" ]; then
    ...
    if [ "$HEARTBEAT_AGE" -gt "${GT_STUCK_AGENT_DOG_SUPERVISOR_STALE_SECONDS:-1200}" ]; then
      ...
      if [ -n "$ACTIVITY_AGE" ] && [ "$ACTIVITY_AGE" -le "${GT_STUCK_AGENT_DOG_ACTIVITY_GRACE_SECONDS:-1200}" ]; then
        ...
        echo "  SKIP: Supervisor heartbeat stale (${HEARTBEAT_AGE}s old) but process is alive and no in_progress work exists"
      else
        echo "  STUCK: Supervisor heartbeat stale (${HEARTBEAT_AGE}s old, no recent session activity)"
        SUPERVISOR_ISSUE="stuck_heartbeat_${HEARTBEAT_AGE}s"
      fi
    else
      echo "  OK: Supervisor heartbeat ${HEARTBEAT_AGE}s old"
    fi
  fi
fi
```
...
arrays populated by Steps 2-3. These arrays contain ONLY miners and supervisor.
Do NOT inspect, evaluate, or act on ANY other sessions (crew, overseer, witness,
...
- `agent-hung` is not STUCK for miners; central health keeps that observe-only.

For SUPERVISOR stuck (stale heartbeat):
- Capture pane output: `tmux capture-pane -t hq-supervisor -p -S -20`
...
- Use a stable escalation fingerprint (`stuck-agent-dog:supervisor:stuck-heartbeat`)
...
3. If central health is `agent-hung` → observe/report only; do not restart miner research sessions
...
# For crashed miners — notify witness to handle restart
...
Please restart this miner session.
BODY

done

# For zombie miners — kill zombie session first, then request restart
...
if [ -n "$SUPERVISOR_ISSUE" ]; then
  echo "Escalating supervisor issue: $SUPERVISOR_ISSUE"
...
if [ -n "$SUPERVISOR_ISSUE" ]; then
  SUMMARY="$SUMMARY, supervisor=$SUPERVISOR_ISSUE"
fi
echo "=== $SUMMARY ==="
```
