...
// since is the Mineshaft session start time: only JSONL files modified at or
// after this time are considered, so unrelated Claude instances (user sessions,
// other Mineshaft rigs) running in the same work dir are excluded.
...
func newestJSONLIn(dir string, since time.Time) (string, bool) {
	...
	for _, e := range entries {
		...
		// Skip files older than the Mineshaft session start — they belong to
