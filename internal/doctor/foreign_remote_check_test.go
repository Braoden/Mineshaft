...
func TestForeignRemoteCheck_DetectsForeignRemote(t *testing.T) {
	...
	runGit(t, townDir, "remote", "add", "mineshaft", foreignDir)
	runGit(t, townDir, "fetch", "mineshaft")
	...
	if check.foreignRemotes[0].name != "mineshaft" {
		t.Errorf("expected foreign remote name 'mineshaft', got %q", check.foreignRemotes[0].name)
	}
}
