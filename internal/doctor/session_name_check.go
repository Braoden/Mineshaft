...
// {role_suffix} is a valid Mineshaft role ("witness", "refinery", "crew-{name}").
	...
	return &MalformedSessionNameCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				...
				CheckDescription: "Detect sessions with outdated Mineshaft naming format",
...
func (c *MalformedSessionNameCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if len(malformed) == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: "All Mineshaft sessions use current naming format",
		}
	}
...
// Mineshaft session name (after the rig prefix).
...
func detectLegacySessionNames(sessions []string, reg *session.PrefixRegistry) []sessionRename {
	...
	// Build set of known Mineshaft prefixes for ownership gating.
...
func matchLegacyName(sess string, rigs map[string]string, knownPrefixes map[string]bool) (sessionRename, bool) {
	for rigName, shortPrefix := range rigs {
		...
		// Mineshaft prefix. This prevents matching non-Mineshaft sessions
		...
		// Validate: must be a known Mineshaft role suffix.
...
// isValidRoleSuffix returns true if suffix is a known Mineshaft role identifier.
