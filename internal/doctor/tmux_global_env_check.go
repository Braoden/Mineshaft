...
// TmuxGlobalEnvCheck verifies that MS_TOWN_ROOT is set in the tmux global
// environment. This is needed for run-shell subprocesses (e.g., ms cycle
	...
	return &TmuxGlobalEnvCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				...
				CheckDescription: "Verify MS_TOWN_ROOT is set in tmux global environment",
...
// Run checks that MS_TOWN_ROOT is set correctly in the tmux global environment.
func (c *TmuxGlobalEnvCheck) Run(ctx *CheckContext) *CheckResult {
	...
	val, err := accessor.GetGlobalEnvironment("MS_TOWN_ROOT")
	if err != nil {
		...
		return &CheckResult{
			...
			Message: "MS_TOWN_ROOT not set in tmux global environment",
			Details: []string{
				"The daemon sets MS_TOWN_ROOT in tmux global env for run-shell subprocesses.",
				"Without it, prefix-based cycle groups (prefix+n/p) fail when CWD is $HOME.",
			},
			FixHint: "Run 'ms doctor --fix' to set MS_TOWN_ROOT in tmux global env",
		}
	}
	...
	if val != ctx.TownRoot {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusWarning,
			Message: fmt.Sprintf("MS_TOWN_ROOT mismatch in tmux global env: %q (expected %q)", val, ctx.TownRoot),
			Details: []string{
				"The daemon sets MS_TOWN_ROOT in tmux global env for run-shell subprocesses.",
				"Without it, prefix-based cycle groups (prefix+n/p) fail when CWD is $HOME.",
			},
	...
	return &CheckResult{
		Name:    c.Name(),
		Status:  StatusOK,
		Message: fmt.Sprintf("MS_TOWN_ROOT=%s in tmux global env", val),
	}
}

// Fix sets MS_TOWN_ROOT in the tmux global environment.
func (c *TmuxGlobalEnvCheck) Fix(ctx *CheckContext) error {
	accessor := c.accessor
	if accessor == nil {
		accessor = tmux.NewTmux()
	}
	return accessor.SetGlobalEnvironment("MS_TOWN_ROOT", ctx.TownRoot)
}
