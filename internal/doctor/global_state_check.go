// ABOUTME: Doctor check for Mineshaft global state configuration.
...
func NewGlobalStateCheck() *GlobalStateCheck {
	return &GlobalStateCheck{
		BaseCheck: BaseCheck{
			...
			CheckDescription: "Validates Mineshaft global state and shell integration",
...
func (c *GlobalStateCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if s.Enabled {
		details = append(details, "Mineshaft: enabled")
	} else {
		details = append(details, "Mineshaft: disabled")
		warnings = append(warnings, "Mineshaft is disabled globally")
	}
...
// files that source the Mineshaft hook script).
