...
func TestCleanupStatusBlocker(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.status, func(t *testing.T) {
			got := cleanupStatusBlocker(miner.CleanupStatus(tt.status))
			if got != tt.want {
...
func TestCleanupStatusBlockerForRecovery_PartialSpawnWithoutHook(t *testing.T) {
	tests := []struct {
		name         string
		status       miner.CleanupStatus
		partialSpawn bool
		want         string
	}{
		{name: "missing cleanup is safe for partial spawn", partialSpawn: true, want: ""},
		{name: "unknown cleanup is safe for partial spawn", status: miner.CleanupUnknown, partialSpawn: true, want: ""},
		{name: "dirty cleanup still blocks partial spawn", status: miner.CleanupUnpushed, partialSpawn: true, want: "cleanup_status=has_unpushed"},
		{name: "missing cleanup still blocks ordinary miner", partialSpawn: false, want: "cleanup_status=<missing>"},
	}
...
func TestStaleCleanupStatusCanBeIgnoredForRecovery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			status:       miner.CleanupUncommitted,
			workTerminal: true,
			hookSafe:     true,
			activeMRSafe: true,
			gitSafe:      true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := miner.CanIgnoreStaleCleanupStatus(tt.status, tt.workTerminal, tt.hookSafe, tt.activeMRSafe, tt.gitSafe)
...
func TestReconcileCleanupStatusIfSafe(t *testing.T) {
	for _, previous := range []miner.CleanupStatus{miner.CleanupUnpushed, miner.CleanupStash, miner.CleanupUncommitted} {
...
func TestReconcileCleanupStatusIfSafe_FailsClosed(t *testing.T) {
	status := &RecoveryStatus{
		CleanupStatus: miner.CleanupUnpushed,
		Verdict:       "SAFE_TO_NUKE",
		Branch:        "miner/nitro",
		MQStatus:      "submitted",
	}
...
func TestCleanupStatusReconcileCandidateRequiresStrictPredicates(t *testing.T) {
	baseStatus := &RecoveryStatus{Verdict: "SAFE_TO_NUKE", Branch: "miner/nitro", MQStatus: "submitted"}
	baseMiner := &miner.Miner{State: miner.StateIdle}
	baseFields := &beads.AgentFields{AgentState: string(beads.AgentStateIdle), CleanupStatus: string(miner.CleanupUnpushed)}
	...
	tests := []struct {
		...
		{name: "stale clean is not rewritten", status: baseStatus, p: baseMiner, fields: &beads.AgentFields{AgentState: string(beads.AgentStateIdle), CleanupStatus: string(miner.CleanupClean)}},
		{name: "working miner blocks", status: baseStatus, p: &miner.Miner{State: miner.StateWorking}, fields: baseFields},
		{name: "working agent bead blocks", status: baseStatus, p: baseMiner, fields: &beads.AgentFields{AgentState: string(beads.AgentStateWorking), CleanupStatus: string(miner.CleanupUnpushed)}},
		{name: "needs recovery blocks", status: &RecoveryStatus{Verdict: "NEEDS_RECOVERY", NeedsRecovery: true, Branch: "miner/nitro", MQStatus: "submitted"}, p: baseMiner, fields: baseFields},
		{name: "unknown mq blocks", status: &RecoveryStatus{Verdict: "SAFE_TO_NUKE", Branch: "miner/nitro", MQStatus: "unknown"}, p: baseMiner, fields: baseFields},
	}
...
func TestRecoveryGitStateBlocker(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: "git_state=unknown path=/tmp/miner: git failed",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := recoveryGitStateBlocker("/tmp/miner", tt.state, tt.err)
			if got != tt.want {
...
func TestStaleCleanWithRealUnpushedStillBlocks(t *testing.T) {
	status := RecoveryStatus{CleanupStatus: miner.CleanupClean}
	if blocker := recoveryGitStateBlocker("/tmp/miner", &GitState{UnpushedCommits: 1}, nil); blocker != "" {
...
func TestDryRunNukeSummary(t *testing.T) {
	tests := []struct {
		...
		{name: "safe", total: 2, want: "Would nuke 2 miner(s)."},
		{name: "blocked", total: 2, blocked: 1, want: "Would refuse to nuke 1 of 2 miner(s) without --force."},
	}
...
func TestHasSubmittableWorkForRecoveryIgnoresPatchEquivalentBranch(t *testing.T) {
	repo := setupRecoveryGitRepo(t)
	runGit(t, repo, "switch", "-c", "miner/equivalent")
	...
	runGit(t, repo, "cherry-pick", "miner/equivalent")
	runGit(t, repo, "push", "origin", "integration/test")
	runGit(t, repo, "switch", "miner/equivalent")
...
func TestHasSubmittableWorkForRecoveryUsesExplicitTargetAncestor(t *testing.T) {
	repo := setupRecoveryGitRepo(t)
	runGit(t, repo, "switch", "-c", "miner/contained")
	...
	runGit(t, repo, "switch", "integration/test")
	runGit(t, repo, "merge", "--ff-only", "miner/contained")
	runGit(t, repo, "push", "origin", "integration/test")
	runGit(t, repo, "switch", "miner/contained")
...
func TestHasSubmittableWorkForRecoveryUsesExplicitTargetCherry(t *testing.T) {
	repo := setupRecoveryGitRepo(t)
	runGit(t, repo, "switch", "-c", "miner/cherry")
	...
	runGit(t, repo, "cherry-pick", "miner/cherry")
	runGit(t, repo, "push", "origin", "integration/test")
	runGit(t, repo, "switch", "miner/cherry")
...
func TestHasSubmittableWorkForRecoveryKeepsExplicitTargetUniquePatch(t *testing.T) {
	repo := setupRecoveryGitRepo(t)
	runGit(t, repo, "switch", "-c", "miner/unique")
