...
var shellCmd = &cobra.Command{
	...
	Long: `Manage the Excavation Site shell integration hook.
...
var shellInstallCmd = &cobra.Command{
	...
	Long: `Install or update the Excavation Site shell integration.

This adds a hook to your shell RC file that:
  - Sets GT_TOWN_ROOT and GT_RIG when you cd into a Excavation Site rig
  - Offers to add new git repos to Excavation Site on first visit
...
var shellRemoveCmd = &cobra.Command{
	...
	Long: `Remove the Excavation Site shell integration from your shell RC file.
...
var shellStatusCmd = &cobra.Command{
	...
	Long: `Show whether Excavation Site is enabled and whether shell integration is installed.
...
func runShellStatus(cmd *cobra.Command, args []string) error {
	s, err := state.Load()
	if err != nil {
		fmt.Println("Excavation Site: not configured")
		fmt.Println("Shell integration: not installed")
		return nil
	}

	if s.Enabled {
		fmt.Println("Excavation Site: enabled")
	} else {
		fmt.Println("Excavation Site: disabled")
	}
