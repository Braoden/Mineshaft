...
human or overseer attention. Escalations are tracked as beads with ms:escalation label.
...
WORKFLOW:
  ...
  2. Runs: ms escalate "Description" --severity high --reason "details"
  3. Escalation is routed based on settings/escalation.json
  4. Recipient acknowledges with: ms escalate ack <id>
  5. After resolution: ms escalate close <id> --reason "fixed"

CONFIGURATION:
  Routing is configured in ~/ms/settings/escalation.json:
...
Examples:
  ms escalate "Build failing" --severity critical --reason "CI blocked"
  ms escalate "Need API credentials" --severity high --source "plugin:rebuild-ms"
  ms escalate "Supervisor await-signal timeout" --severity medium --source supervisor --fingerprint supervisor:await-signal:hq-supervisor
  ms escalate "Code review requested" --reason "PR #123 ready"
  ms escalate list                          # Show open escalations
  ms escalate ack hq-abc123                 # Acknowledge
  ms escalate close hq-abc123 --reason "Fixed in commit abc"
  ms escalate stale                         # Re-escalate stale escalations`,
}
...
Examples:
  ms escalate list              # Open escalations only
  ms escalate list --all        # Include closed
  ms escalate list --json       # JSON output`,
...
Examples:
  ms escalate close hq-abc123 --reason "Fixed in commit abc"
  ms escalate close hq-abc123 --reason "Not reproducible"`,
	Args: cobra.ExactArgs(1),
...
Examples:
  ms escalate stale              # Re-escalate stale escalations
  ms escalate stale --dry-run    # Show what would be done
  ms escalate stale --json       # JSON output of results`,
...
Examples:
  ms escalate show hq-abc123
  ms escalate show hq-abc123 --json`,
	Args: cobra.ExactArgs(1),
...
func init() {
	...
	escalateCmd.Flags().StringVar(&escalateSource, "source", "", "Source identifier (e.g., plugin:rebuild-ms, patrol:supervisor)")
