...
A mountain is a minecart with the 'mountain' label. This command:
  1. Stages the minecart (validate DAG, compute waves)
  2. Adds the 'mountain' label (enables Supervisor audit + Witness failure tracking)
  3. Launches the minecart (dispatches Wave 1)

Regular minecarts (no mountain label) continue working as normal.
...
var mountainStatusCmd = &cobra.Command{
	Use:   "status [epic-id|minecart-id]",
...
Accepts either an epic ID or minecart ID.`,
	Args: cobra.MaximumNArgs(1),
	RunE: runMountainStatus,
}

var mountainPauseCmd = &cobra.Command{
	Use:   "pause <epic-id|minecart-id>",
...
var mountainResumeCmd = &cobra.Command{
	Use:   "resume <epic-id|minecart-id>",
...
var mountainCancelCmd = &cobra.Command{
	Use:   "cancel <epic-id|minecart-id>",
	Short: "Cancel a mountain (remove label, keep minecart)",
	Long: `Cancel the Mountain-Eater on a minecart. Removes the mountain label
but leaves the minecart for manual management. Active miners continue
...
func runMountain(cmd *cobra.Command, args []string) error {
	...
	dag := buildMinecartDAG(beadList, deps)
	...
	if status == minecartStatusStagedWarnings && !mountainForce {
		return fmt.Errorf("staging has warnings, use --force to proceed")
	}

	// Step 3: Create the staged minecart.
	title := "Mountain: " + result.Title
	minecartID, err := createStagedMinecart(dag, waves, status, title)
	if err != nil {
		return fmt.Errorf("create minecart: %w", err)
	}

	fmt.Printf("\nCreating minecart...\n")
	fmt.Printf("  Minecart: %s %q\n", minecartID, title)
	fmt.Printf("  Label: mountain\n")

	// Step 4: Add the mountain label.
	if err := bdAddLabelTown(minecartID, "mountain"); err != nil {
		return fmt.Errorf("add mountain label: %w", err)
	}

	// Step 5: Launch — transition to open + dispatch Wave 1.
	if err := transitionMinecartToOpen(minecartID, true); err != nil {
		return fmt.Errorf("launch minecart: %w", err)
	}
...
// mountainMinecartInfo holds minecart data for mountain status display.
type mountainMinecartInfo struct {
	ID     string   `json:"id"`
...
func showAllMountainStatus(townBeads string) error {
	minecarts, err := findMountainMinecarts(townBeads)
	if err != nil {
		return err
	}
	...
	for _, c := range minecarts {
		...
		trackedBeads, _, err := collectMinecartBeads(c.ID)
		if err != nil {
...
func showMountainDetail(townBeads, inputID string) error {
	// Resolve: inputID could be an epic or minecart.
	minecartID, err := resolveMountainID(townBeads, inputID)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("minecart %q not found", minecartID)
	}
	...
	if !hasLabel(cv.Labels, "mountain") {
		return fmt.Errorf("%s is not a mountain (no mountain label)", minecartID)
	}

	// Collect tracked beads.
	trackedBeads, deps, err := collectMinecartBeads(minecartID)
	if err != nil {
		return fmt.Errorf("reading tracked beads: %w", err)
	}

	dag := buildMinecartDAG(trackedBeads, deps)
...
// findMountainMinecarts lists all open minecarts with the mountain label.
func findMountainMinecarts(townBeads string) ([]mountainMinecartInfo, error) {
	issues, err := listMinecartIssues(townBeads, "open", false, "mountain")
	if err != nil {
		return nil, fmt.Errorf("listing mountain minecarts: %w", err)
	}

	minecarts := make([]mountainMinecartInfo, 0, len(issues))
	for _, issue := range issues {
		minecarts = append(minecarts, mountainMinecartInfo{
			ID:     issue.ID,
			Title:  issue.Title,
			Labels: issue.Labels,
		})
...
// resolveMountainID resolves an epic-id or minecart-id to the mountain minecart ID.
// If the input is already a minecart with the mountain label, returns it directly.
// If the input is an epic, searches for a mountain minecart tracking it.
func resolveMountainID(townBeads, inputID string) (string, error) {
	...
	// Input is an epic — find a mountain minecart that tracks it.
	minecarts, err := findMountainMinecarts(townBeads)
	if err != nil {
		return "", err
	}

	for _, cv := range minecarts {
		// Check if the minecart title starts with "Mountain: " + epic title.
...
// runMountainPause pauses an active mountain by setting the minecart to paused status.
func runMountainPause(cmd *cobra.Command, args []string) error {
	...
	minecartID, err := resolveMountainID(townBeads, args[0])
	if err != nil {
		return err
	}

	// Add paused label — the MinecartManager checks this to skip dispatch.
	if err := bdAddLabelTown(minecartID, "mountain:paused"); err != nil {
...
func runMountainResume(cmd *cobra.Command, args []string) error {
	...
	if err := bdRemoveLabelTown(minecartID, "mountain:paused"); err != nil {
		return fmt.Errorf("resume mountain: %w", err)
	}

	fmt.Printf("Mountain %s resumed.\n", minecartID)
	fmt.Printf("MinecartManager will continue dispatching waves.\n")
	return nil
}

// runMountainCancel cancels a mountain by removing the mountain label.
// Leaves the minecart intact for manual management.
func runMountainCancel(cmd *cobra.Command, args []string) error {
	...
	if err := bdRemoveLabelTown(minecartID, "mountain"); err != nil {
		return fmt.Errorf("cancel mountain: %w", err)
	}
	// Best-effort remove paused label too.
	_ = bdRemoveLabelTown(minecartID, "mountain:paused")
