...
func TestParseAgentBeadLabels(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			stdout:     []byte(`[{"id":"ms-test","labels":["idle:3","ms:agent"]}]`),
			stderr:     nil,
			agentBead:  "ms-test",
			wantLabels: []string{"idle:3", "ms:agent"},
			wantErr:    "",
		},
		{
			name:       "valid response with no labels",
			stdout:     []byte(`[{"id":"ms-test","labels":[]}]`),
			stderr:     nil,
			agentBead:  "ms-test",
			wantLabels: []string{},
			wantErr:    "",
		},
		{
			...
			stdout:     []byte(`[{"id":"ms-test","labels":null}]`),
			stderr:     nil,
			agentBead:  "ms-test",
			wantLabels: nil,
			wantErr:    "",
		...
		},
		{
			...
			stderr:    nil,
			agentBead: "ms-test",
			wantErr:   "agent bead query returned no output: ms-test",
		},
		{
			...
			stderr:    nil,
			agentBead: "ms-test",
			wantErr:   "agent bead not found: ms-test",
		},
		{
			name:      "invalid JSON",
