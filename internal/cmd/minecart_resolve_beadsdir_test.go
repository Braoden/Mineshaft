...
// TestMinecartResolveBeadsDir_RegressionEmptyMinecart is a regression test for
...
// appear empty.
//
// Fix: minecart.go and minecart_stage.go now call beads.ResolveBeadsDir(townBeads)
// before passing to EnsureCustomTypes/EnsureCustomStatuses.
func TestMinecartResolveBeadsDir_RegressionEmptyMinecart(t *testing.T) {
	...
	t.Run("getTownBeadsDir returns workspace root not beads dir", func(t *testing.T) {
		...
		if result == beadsDir {
			t.Log("getTownBeadsDir now returns .beads directly — " +
				"ResolveBeadsDir call in minecart code is redundant but harmless")
			return
		}
...
// redirect files, which is how rig worktrees (miners) point back to the
// shared beads database. The minecart code must call ResolveBeadsDir to handle
...
// TestMinecartCreate_SentinelPlacement verifies that the minecart create path
// writes sentinel files to the .beads directory, not the workspace root.
// This is an end-to-end regression test for the empty minecart bug.
func TestMinecartCreate_SentinelPlacement(t *testing.T) {
	if runtime.GOOS == "windows" {
