...
func TestStartMinersWithWorkSkipsDotDirs(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(rigPath, "miners", ".claude"), 0755); err != nil {
		t.Fatalf("mkdir .claude miner: %v", err)
	}
...
`
	...
	started, errs := startMinersWithWork(townRoot, rigName)

	if len(started) != 0 {
		t.Fatalf("expected no miners started, got %v", started)
	}
	if len(errs) != 0 {
