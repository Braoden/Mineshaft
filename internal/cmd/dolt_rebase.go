...
Unlike 'ms dolt flatten' (which destroys ALL history), surgical rebase
...
Flatten mode (ms dolt flatten) is safe with concurrent writes.
...
func runDoltRebase(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		fmt.Printf("  Branch swap already complete — verify manually with 'ms dolt status'\n")
	} else {
		integrityOK := true
