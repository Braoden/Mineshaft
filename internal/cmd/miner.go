...
var minerCmd = &cobra.Command{
	Use:     "miner",
	Aliases: []string{"miners"},
	GroupID: GroupAgents,
	Short:   "Manage miners (persistent identity, ephemeral sessions)",
	RunE:    requireSubcommand,
	Long: `Manage miner lifecycle in rigs.

Miners have PERSISTENT IDENTITY but EPHEMERAL SESSIONS. Each miner has
...
var minerListCmd = &cobra.Command{
	Use:   "list [rig]",
	Short: "List miners in a rig",
	Long: `List miners in a rig or all rigs.

In the transient model, miners exist only while working. The list shows
all miners with their states:
...
var minerRemoveCmd = &cobra.Command{
	Use:   "remove <rig>/<miner>... | <rig> --all",
	Short: "Remove miners from a rig",
	Long: `Remove one or more miners from a rig.
...
var minerStatusCmd = &cobra.Command{
	...
	Short: "Show detailed status for a miner",
	Long: `Show detailed status for a miner.
...
NOTE: The argument is <rig>/<miner> — a single argument with a slash
...
var (
	minerStatusJSON                    bool
	minerGitStateJSON                  bool
	minerGCDryRun                      bool
	minerNukeAll                       bool
	minerNukeDryRun                    bool
	minerNukeForce                     bool
	minerCheckRecoveryJSON             bool
	minerCheckRecoveryReconcileCleanup bool
	minerPoolInitDryRun                bool
	minerPoolInitSize                  int
)

var minerGCCmd = &cobra.Command{
	Use:   "gc <rig>",
	Short: "Garbage collect stale miner branches",
	Long: `Garbage collect stale miner branches in a rig.

Miners use unique timestamped branches (miner/<name>-<timestamp>) to
prevent drift issues. Over time, these branches accumulate when stale
miners are repaired.

This command removes orphaned branches:
  - Branches for miners that no longer exist
...
var minerNukeCmd = &cobra.Command{
	Use:   "nuke <rig>/<miner>... | <rig> --all",
	Short: "Completely destroy a miner (session, worktree, branch, agent bead)",
	Long: `Completely destroy a miner and all its artifacts.
...
This is the nuclear option for post-merge cleanup. It:
  ...
  3. Deletes the miner branch
  4. Closes the agent bead (if exists)

SAFETY CHECKS: The command refuses to nuke a miner if:
  - cleanup_status is dirty, unknown, or missing
  - Worktree fallback detects unpushed/uncommitted/stashed changes
  - Miner has an open merge request (MR bead or active_mr)
  - Miner has work on its hook
...
var minerGitStateCmd = &cobra.Command{
	Use:   "git-state <rig>/<miner>",
	Short: "Show git state for pre-kill verification",
	Long: `Show git state for a miner's worktree.
...
var minerCheckRecoveryCmd = &cobra.Command{
	Use:   "check-recovery <rig>/<miner>",
	Short: "Check if miner needs recovery vs safe to nuke",
	Long: `Check recovery status of a miner based on cleanup_status, active_mr, and merge queue state.
...
This prevents accidental data loss when cleaning up dormant miners.
...
var (
	minerStaleJSON      bool
	minerStaleThreshold int
	minerStaleCleanup   bool
	minerStaleDryRun    bool
	minerPruneDryRun    bool
	minerPruneRemote    bool
)

var minerStaleCmd = &cobra.Command{
	Use:   "stale <rig>",
	Short: "Detect stale miners that may need cleanup",
	Long: `Detect stale miners in a rig that are candidates for cleanup.

A miner is considered stale if:
...
Use --cleanup to automatically nuke stale miners that are safe to remove.
...
var minerPruneCmd = &cobra.Command{
	Use:   "prune <rig>",
	Short: "Prune stale miner branches (local and remote)",
	Long: `Prune stale miner branches in a rig.

Finds and deletes miner branches that are no longer needed:
  - Branches fully merged to main
  - Branches whose remote tracking branch was deleted (post-merge cleanup)
  - Branches for miners that no longer exist (orphaned)

Uses safe deletion (git branch -d) — only removes fully merged branches.
Also cleans up remote miner branches that are fully merged.
...
var minerPoolInitCmd = &cobra.Command{
	Use:   "pool-init <rig>",
	Short: "Initialize a persistent miner pool for a rig",
...
Pool size is determined by (in priority order):
  1. --size flag
  2. miner_pool_size in rig config.json
  3. Default: 4

Miner names come from:
  1. miner_names in rig config.json (if specified)
  2. The rig's name pool theme (default: mad-max)

Existing miners are preserved — only new ones are created
...
func init() {
	// List flags
	minerListCmd.Flags().BoolVar(&minerListJSON, "json", false, "Output as JSON")
	minerListCmd.Flags().BoolVar(&minerListAll, "all", false, "List miners in all rigs")

	// Remove flags
	minerRemoveCmd.Flags().BoolVarP(&minerForce, "force", "f", false, "Force removal, bypassing checks")
	minerRemoveCmd.Flags().BoolVar(&minerRemoveAll, "all", false, "Remove all miners in the rig")

	// Status flags
	minerStatusCmd.Flags().BoolVar(&minerStatusJSON, "json", false, "Output as JSON")

	// Git-state flags
	minerGitStateCmd.Flags().BoolVar(&minerGitStateJSON, "json", false, "Output as JSON")

	// GC flags
	minerGCCmd.Flags().BoolVar(&minerGCDryRun, "dry-run", false, "Show what would be deleted without deleting")

	// Nuke flags
	minerNukeCmd.Flags().BoolVar(&minerNukeAll, "all", false, "Nuke all miners in the rig")
	minerNukeCmd.Flags().BoolVar(&minerNukeDryRun, "dry-run", false, "Show what would be nuked without doing it")
	minerNukeCmd.Flags().BoolVarP(&minerNukeForce, "force", "f", false, "Force nuke, bypassing all safety checks (LOSES WORK)")

	// Check-recovery flags
	minerCheckRecoveryCmd.Flags().BoolVar(&minerCheckRecoveryJSON, "json", false, "Output as JSON")
	minerCheckRecoveryCmd.Flags().BoolVar(&minerCheckRecoveryReconcileCleanup, "reconcile-cleanup", false, "Safely rewrite stale dirty cleanup_status to clean when live recovery predicates prove no work is at risk")

	// Stale flags
	minerStaleCmd.Flags().BoolVar(&minerStaleJSON, "json", false, "Output as JSON")
	minerStaleCmd.Flags().IntVar(&minerStaleThreshold, "threshold", 20, "Commits behind main to consider stale")
	minerStaleCmd.Flags().BoolVar(&minerStaleCleanup, "cleanup", false, "Automatically nuke stale miners")
	minerStaleCmd.Flags().BoolVar(&minerStaleDryRun, "dry-run", false, "Show what would be cleaned without doing it")

	// Prune flags
	minerPruneCmd.Flags().BoolVar(&minerPruneDryRun, "dry-run", false, "Show what would be pruned without doing it")
	minerPruneCmd.Flags().BoolVar(&minerPruneRemote, "remote", false, "Also prune remote miner branches on origin")

	// Pool-init flags
	minerPoolInitCmd.Flags().BoolVar(&minerPoolInitDryRun, "dry-run", false, "Show what would be created without doing it")
	minerPoolInitCmd.Flags().IntVar(&minerPoolInitSize, "size", 0, "Pool size (overrides rig config)")

	// Add subcommands
	minerCmd.AddCommand(minerListCmd)
	minerCmd.AddCommand(minerAddCmd)
	minerCmd.AddCommand(minerRemoveCmd)
	minerCmd.AddCommand(minerStatusCmd)
	minerCmd.AddCommand(minerGitStateCmd)
	minerCmd.AddCommand(minerCheckRecoveryCmd)
	minerCmd.AddCommand(minerGCCmd)
	minerCmd.AddCommand(minerNukeCmd)
	minerCmd.AddCommand(minerStaleCmd)
	minerCmd.AddCommand(minerPruneCmd)
	minerCmd.AddCommand(minerPoolInitCmd)

	rootCmd.AddCommand(minerCmd)
}

// MinerListItem represents a miner in list output.
type MinerListItem struct {
	Rig                  string        `json:"rig"`
	Name                 string        `json:"name"`
	State                miner.State `json:"state"`
...
// effectiveMinerState returns the observable state used by miner list output.
// Active work is ground truth for working; tmux liveness alone is not enough
// because persistent miners may keep a reusable live session after completion.
// Zombie entries are never auto-rewritten.
func effectiveMinerState(item MinerListItem) miner.State {
	state := item.State
	...
	if item.SessionRunning && item.Issue != "" && (state == miner.StateDone || state == miner.StateIdle) {
		return miner.StateWorking
	}
	...
	if !item.SessionRunning && !item.Zombie && state == miner.StateWorking {
		return miner.StateStalled
	}
	return state
}
...
func activeMRBlocksReuse(bd reuseMRShower, mrID, sourceHint string, requireGitSafe, gitSafe bool) bool {
	assessment := miner.AssessActiveMR(bd, miner.ActiveMRInput{ActiveMR: mrID, SourceIssueHint: sourceHint, RequireGitSafe: requireGitSafe, GitSafe: gitSafe})
	return assessment.Pending
}

func minerReuseStatus(state miner.State, cleanupStatus, activeMR, branch string, activeMRBlocks, staleCleanupSafe bool) string {
	input := miner.WorkstateInput{State: state, CleanupStatus: miner.CleanupStatus(cleanupStatus), ActiveMR: activeMR, Branch: branch}
	if activeMRBlocks {
	...
	return miner.DecideWorkstate(input).ReuseStatus
}

// getMinerManager creates a miner manager for the given rig.
func getMinerManager(rigName string) (*miner.Manager, *rig.Rig, error) {
	...
	t := tmux.NewTmux()
	mgr := miner.NewManager(r, minerGit, t)

	return mgr, r, nil
}

func runMinerList(cmd *cobra.Command, args []string) error {
	var rigs []*rig.Rig
	...
	if minerListAll {
		...
		_, r, err := getMinerManager(args[0])
		if err != nil {
			return err
		}
		rigs = []*rig.Rig{r}
	...
	// Collect miners from all rigs
	t := tmux.NewTmux()
	allMiners := make([]MinerListItem, 0)
	...
	for _, r := range rigs {
		...
		miners, err := mgr.List()
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: failed to list miners in %s: %v\n", r.Name, err)
			continue
		}

		// Track known miner names from filesystem for zombie detection
		knownNames := make(map[string]bool)
		for _, p := range miners {
			running, _ := minerMgr.IsRunning(p.Name)
			cleanupStatus := ""
			activeMR := ""
			agentBeadID := minerBeadIDForRig(r, r.Name, p.Name)
			if _, fields, err := bd.GetAgentBead(agentBeadID); err == nil && fields != nil {
				cleanupStatus = fields.CleanupStatus
				activeMR = fields.ActiveMR
			}
			state := effectiveMinerState(MinerListItem{
				State:          p.State,
				Issue:          p.Issue,
				SessionRunning: running,
			})
			disposition := mgr.WorkstateDispositionForMiner(p.Name, state, p.Issue)
			allMiners = append(allMiners, MinerListItem{
				Rig:                  r.Name,
				Name:                 p.Name,
				State:                state,
				Issue:                p.Issue,
		...
		zombieSessions, _ := findRigMinerSessions(r.Name)
		for _, sessionName := range zombieSessions {
			_, minerName, ok := parseMinerSessionName(sessionName)
			if !ok {
				continue
			}
			if !knownNames[minerName] {
				allMiners = append(allMiners, MinerListItem{
					Rig:            r.Name,
					Name:           minerName,
	...
	fmt.Printf("%s\n\n", style.Bold.Render("Miners"))
	for _, p := range allMiners {
		// Session indicator
...
func runMinerAdd(cmd *cobra.Command, args []string) error {
	...
	mgr, _, err := getMinerManager(rigName)
	if err != nil {
		return err
	}

	fmt.Printf("Adding miner %s to rig %s...\n", minerName, rigName)

	p, err := mgr.Add(minerName)
	if err != nil {
		return fmt.Errorf("adding miner: %w", err)
	}

	fmt.Printf("%s Miner %s added.\n", style.SuccessPrefix, p.Name)
...
func runMinerRemove(cmd *cobra.Command, args []string) error {
	targets, err := resolveMinerTargets(args, minerRemoveAll)
	if err != nil {
		return err
	}

	if len(targets) == 0 {
		fmt.Println("No miners to remove.")
		return nil
	}
	...
	for _, p := range targets {
		...
		if !minerForce {
			minerMgr := miner.NewSessionManager(t, p.r)
			running, _ := minerMgr.IsRunning(p.minerName)
			if running {
				removeErrors = append(removeErrors, fmt.Sprintf("%s/%s: session is running (stop first or use --force)", p.rigName, p.minerName))
				continue
			}
		}

		fmt.Printf("Removing miner %s/%s...\n", p.rigName, p.minerName)

		if err := p.mgr.Remove(p.minerName, minerForce); err != nil {
			if errors.Is(err, miner.ErrHasChanges) {
				removeErrors = append(removeErrors, fmt.Sprintf("%s/%s: has uncommitted changes (use --force)", p.rigName, p.minerName))
			} else {
				removeErrors = append(removeErrors, fmt.Sprintf("%s/%s: %v", p.rigName, p.minerName, err))
			}
			continue
		}
...
// MinerStatus represents detailed miner status for JSON output.
...
func runMinerStatus(cmd *cobra.Command, args []string) error {
	...
	// Get miner info
	p, err := mgr.Get(minerName)
	if err != nil {
		return fmt.Errorf("miner '%s' not found in rig '%s'", minerName, rigName)
	}

	// Get session info
	t := tmux.NewTmux()
	minerMgr := miner.NewSessionManager(t, r)
	sessInfo, err := minerMgr.Status(minerName)
	if err != nil {
		// Non-fatal - continue without session info
		sessInfo = &miner.SessionInfo{
			Miner: minerName,
			Running: false,
		}
	}
	...
	fmt.Printf("%s\n\n", style.Bold.Render(fmt.Sprintf("Miner: %s/%s", rigName, minerName)))
...
// GitState represents the git state of a miner's worktree.
type GitState struct {
	...
	StashCount            int      `json:"stash_count"`                  // Current-branch stashes: per-miner risk.
	SharedStashCount      int      `json:"shared_stash_count,omitempty"` // Other branch stashes visible through the shared repo.
}

func runMinerGitState(cmd *cobra.Command, args []string) error {
	rigName, minerName, err := parseAddress(args[0])
	if err != nil {
		return err
	}
	...
	// Get git state from the miner's worktree
	...
	// Human-readable output
	fmt.Printf("%s\n\n", style.Bold.Render(fmt.Sprintf("Git State: %s/%s", r.Name, minerName)))
...
// RecoveryStatus represents whether a miner needs recovery or is safe to nuke.
type RecoveryStatus struct {
	Rig                  string                `json:"rig"`
	Miner              string                `json:"miner"`
	CleanupStatus        miner.CleanupStatus `json:"cleanup_status"`
	NeedsRecovery        bool                  `json:"needs_recovery"`
...
func runMinerCheckRecovery(cmd *cobra.Command, args []string) error {
	rigName, minerName, err := parseAddress(args[0])
	if err != nil {
		return err
	}
	...
	// Verify miner exists and get info
	p, err := mgr.Get(minerName)
	if err != nil {
		return fmt.Errorf("miner '%s' not found in rig '%s'", minerName, rigName)
	}
	...
	agentBeadID := minerBeadIDForRig(r, rigName, minerName)
	...
	input := miner.WorkstateInput{State: p.State, CleanupStatus: miner.CleanupUnknown, Branch: p.Branch}
	...
	if err != nil || fields == nil {
		...
		loadGitState()
		if gitErr != nil {
			input.CleanupStatus = miner.CleanupUnknown
			...
			input.CleanupStatus = miner.CleanupClean
		} else if gitState.UnpushedCommits > 0 {
			input.CleanupStatus = miner.CleanupUnpushed
			input.UnpushedCommits = gitState.UnpushedCommits
		} else if gitState.StashCount > 0 {
			input.CleanupStatus = miner.CleanupStash
			input.StashCount = gitState.StashCount
		} else {
			input.CleanupStatus = miner.CleanupUncommitted
			input.GitDirty = true
		...
		if fields.ActiveMR != "" {
			...
			activeMRAssessment = miner.AssessActiveMR(bd, miner.ActiveMRInput{
				ActiveMR:        fields.ActiveMR,
		...
		if blocker := cleanupStatusBlockerForRecovery(input.CleanupStatus, partialSpawn); blocker == "" && !input.CleanupStatus.IsSafe() {
			...
			if input.CleanupStatus == miner.CleanupUnpushed {
				loadGitState()
			}
			gitSafe := activeMRGitSafeForWorktree(p.ClonePath)
			if miner.CanIgnoreStaleCleanupStatus(input.CleanupStatus, workTerminal, hookSafe, !activeMRAssessment.Pending, gitSafe) {
				input.IgnoreCleanupStatus = true
	...
	// Human-readable output
	fmt.Printf("%s\n\n", style.Bold.Render(fmt.Sprintf("Recovery Status: %s/%s", rigName, minerName)))
	...
	case "PENDING_MR":
		...
		fmt.Println()
		fmt.Println("  Work is waiting on an active merge request; preserve this miner until it lands.")
	case "NEEDS_RECOVERY":
		...
		fmt.Println("  Escalate to Overseer for recovery before cleanup.")
	default:
...
func applyGitStateToWorkstateInput(input *miner.WorkstateInput, worktreePath string, gitState *GitState, gitErr error) {
	if gitErr != nil {
		input.GitCheckFailed = true
...
func applyMQFactsToWorkstateInput(input *miner.WorkstateInput, status *RecoveryStatus, bd *beads.Beads, beadTerminal bool, worktreePath string, targetRefs []string, gitState *GitState, gitErr error) {
	if status.Branch == "" {
		return
	}
...
func applyWorkstateDispositionToRecoveryStatus(status *RecoveryStatus, disposition miner.WorkstateDisposition) {
...
func cleanupStatusBlocker(status miner.CleanupStatus) string {
	switch status {
	case miner.CleanupClean:
		return ""
	case "":
		return "cleanup_status=<missing>"
	case miner.CleanupUnknown:
...
func cleanupStatusBlockerForRecovery(status miner.CleanupStatus, partialSpawnWithoutHook bool) string {
	if partialSpawnWithoutHook && (status == "" || status == miner.CleanupUnknown) {
		return ""
	}
...
func reconcileCleanupStatusIfSafe(status *RecoveryStatus, updater cleanupStatusUpdater, agentBeadID string, p *miner.Miner, fields *beads.AgentFields) {
	...
	if err := updater.UpdateAgentCleanupStatus(agentBeadID, string(miner.CleanupClean)); err != nil {
		status.NeedsRecovery = true
		status.Verdict = "NEEDS_RECOVERY"
		status.Blockers = append(status.Blockers, fmt.Sprintf("cleanup_reconcile_failed: %v", err))
		return
	}
	status.CleanupStatus = miner.CleanupClean
	status.Reconciled = true
	status.Diagnostics = append(status.Diagnostics, fmt.Sprintf("reconciled_cleanup_status=clean previous=%s", previous))
}

func cleanupStatusReconcileCandidate(status *RecoveryStatus, p *miner.Miner, fields *beads.AgentFields) (miner.CleanupStatus, bool) {
	if status == nil || p == nil || fields == nil {
		return "", false
	}
	previous := miner.CleanupStatus(fields.CleanupStatus)
	if previous == "" || previous == miner.CleanupClean {
		return previous, false
	}
	if p.State != miner.StateIdle || beads.AgentState(fields.AgentState) != beads.AgentStateIdle {
		return previous, false
	}
...
func activeMRBlocker(bd issueShower, mrID, sourceHint string, requireGitSafe, gitSafe bool) string {
	assessment := miner.AssessActiveMR(bd, miner.ActiveMRInput{
		ActiveMR:        mrID,
		SourceIssueHint: sourceHint,
...
// isAssignedBeadTerminal reports whether the miner's assigned bead (if any)
...
// applyMQCheck mutates status based on merge-queue state for the miner's
...
func runMinerGC(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	...
	fmt.Printf("Garbage collecting stale miner branches in %s...\n\n", r.Name)

	if minerGCDryRun {
		// Dry run - list branches that would be deleted
		repoGit := git.NewGit(r.Path)

		// List all miner branches
		branches, err := repoGit.ListBranches("miner/*")
		if err != nil {
			return fmt.Errorf("listing branches: %w", err)
		}

		if len(branches) == 0 {
			fmt.Println("No miner branches found.")
			return nil
		}
...
func runMinerNuke(cmd *cobra.Command, args []string) error {
	targets, err := resolveMinerTargets(args, minerNukeAll)
	if err != nil {
		return err
	}

	if len(targets) == 0 {
		fmt.Println("No miners to nuke.")
		return nil
	}

	// Safety checks: refuse to nuke miners with active work unless --force is set
	if !minerNukeForce && !minerNukeDryRun {
		var blocked []*SafetyCheckResult
		for _, p := range targets {
			result := checkMinerSafety(p)
			if result.Blocked {
		...
		if len(blocked) > 0 {
			...
			return fmt.Errorf("blocked: %d miner(s) failed nuke safety checks: %s", len(blocked), formatSafetyCheckBlockers(blocked))
		}
	}
	...
	nuked := 0
	batchPurge := !minerNukeDryRun && len(targets) > 1
	purgeRigs := make(map[string]*rig.Rig)
	dryRunBlocked := 0

	for _, p := range targets {
		if minerNukeDryRun {
			blocked := !minerNukeForce && checkMinerSafety(p).Blocked
			if blocked {
		...
		if minerNukeForce {
			fmt.Printf("%s Nuking %s/%s (--force)...\n", style.Warning.Render("⚠"), p.rigName, p.minerName)
		} else {
			fmt.Printf("Nuking %s/%s...\n", p.rigName, p.minerName)
		}

		if err := nukeMinerFullWithOptions(p.minerName, p.rigName, p.mgr, p.r, nukeMinerOptions{PurgeClosedEphemerals: !batchPurge}); err != nil {
			nukeErrors = append(nukeErrors, fmt.Sprintf("%s/%s: %v", p.rigName, p.minerName, err))
			continue
		}
...
func dryRunNukeSummary(total, blocked int) string {
	if blocked > 0 {
		return fmt.Sprintf("Would refuse to nuke %d of %d miner(s) without --force.", blocked, total)
	}
	return fmt.Sprintf("Would nuke %d miner(s).", total)
}

// nukeMinerFull performs the complete cleanup sequence for a single miner:
// 1. Kill tmux session
// 2. Delete worktree (via RemoveWithOptions with nuclear=true)
// 3. Delete git branch
// 4. Close agent bead
// This is the canonical cleanup path used by both `miner nuke` and `miner stale --cleanup`.
func nukeMinerFull(minerName, rigName string, mgr *miner.Manager, r *rig.Rig) error {
	return nukeMinerFullWithOptions(minerName, rigName, mgr, r, nukeMinerOptions{PurgeClosedEphemerals: true})
}

type nukeMinerOptions struct {
	PurgeClosedEphemerals bool
}

func nukeMinerFullWithOptions(minerName, rigName string, mgr *miner.Manager, r *rig.Rig, opts nukeMinerOptions) error {
	t := tmux.NewTmux()

	// Step 1: Kill tmux session unconditionally to prevent ghost sessions
	// when IsRunning fails to detect the session.
	sessMgr := miner.NewSessionManager(t, r)
	if err := sessMgr.Stop(minerName, true); err != nil {
		if !errors.Is(err, miner.ErrSessionNotFound) {
	...
	// Step 2: Get miner info before deletion (for branch name + hooked work bead)
	minerInfo, getErr := mgr.Get(minerName)
	var branchToDelete string
	if getErr == nil && minerInfo != nil {
		branchToDelete = minerInfo.Branch
	}
	...
	if branchToDelete != "" {
		...
		// Use ClonePath from the miner record — the worktree lives at
		// <rig>/miners/<name>/<rigName>/, not <rig>/miners/<name>/.
		if minerInfo != nil && minerInfo.ClonePath != "" {
			if _, statErr := os.Stat(minerInfo.ClonePath); statErr == nil {
				pushGit = git.NewGit(minerInfo.ClonePath)
			}
		}
		if pushGit == nil {
	...
	// Step 3: Delete worktree (nuclear=true to bypass safety checks for stale miners)
	if err := mgr.RemoveWithOptions(minerName, true, true, false); err != nil {
		if errors.Is(err, miner.ErrMinerNotFound) {
			fmt.Printf("  %s worktree already gone\n", style.Dim.Render("○"))
			resetMinerAgentBeadForReuse(r, rigName, minerName)
		} else {
	...
	// Without this, closed wisps from mol-miner-work steps, mol-witness-patrol
...
func resetMinerAgentBeadForReuse(r *rig.Rig, rigName, minerName string) {
	agentBeadID := minerBeadIDForRig(r, rigName, minerName)
	bd := beads.New(r.Path)
...
func nukeCleanupMolecules(workBeadID string, r *rig.Rig) {
	...
	if _, detachErr := bd.DetachMoleculeWithAudit(workBeadID, beads.DetachOptions{
		Operation: "burn",
		Reason:    "miner nuked: cleaning stale molecule",
	}); detachErr != nil {
	...
	if closeErr := bd.ForceCloseWithReason("burned: miner nuked", moleculeID); closeErr != nil {
...
func runMinerStale(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	mgr, r, err := getMinerManager(rigName)
	if err != nil {
		return err
	...
	fmt.Printf("Detecting stale miners in %s (threshold: %d commits behind main)...\n\n", r.Name, minerStaleThreshold)

	staleInfos, err := mgr.DetectStaleMiners(minerStaleThreshold)
	if err != nil {
		return fmt.Errorf("detecting stale miners: %w", err)
	}
	...
	for _, info := range staleInfos {
		...
		if info.CommitsBehind > 0 {
			...
			if info.CommitsBehind >= minerStaleThreshold {
	...
	if minerStaleCleanup && staleCount > 0 {
		fmt.Println()
		if minerStaleDryRun {
			fmt.Printf("Would clean up %d stale miner(s):\n", staleCount)
			for _, info := range staleInfos {
				if info.IsStale {
			...
			fmt.Printf("Cleaning up %d stale miner(s)...\n", staleCount)
			nuked := 0
			...
			for _, info := range staleInfos {
				...
				fmt.Printf("Nuking %s...\n", info.Name)
				if err := nukeMinerFullWithOptions(info.Name, rigName, mgr, r, nukeMinerOptions{PurgeClosedEphemerals: !batchPurge}); err != nil {
...
func runMinerPrune(cmd *cobra.Command, args []string) error {
	rigName := args[0]

	_, r, err := getMinerManager(rigName)
	if err != nil {
		return err
	}

	// Use the overseer/rig clone (or bare repo) for branch operations
	var repoGit *git.Git
	bareRepoPath := filepath.Join(r.Path, ".repo.git")
	if info, statErr := os.Stat(bareRepoPath); statErr == nil && info.IsDir() {
		repoGit = git.NewGitWithDir(bareRepoPath, "")
	} else {
		repoGit = git.NewGit(filepath.Join(r.Path, "overseer", "rig"))
	}

	fmt.Printf("Pruning stale miner branches in %s...\n", r.Name)
	...
	pruned, err := repoGit.PruneStaleBranches("miner/*", minerPruneDryRun)
	if err != nil {
		return fmt.Errorf("pruning local branches: %w", err)
	}

	if len(pruned) == 0 {
		fmt.Println("No stale local miner branches found.")
	} else {
		verb := "Pruned"
		if minerPruneDryRun {
			verb = "Would prune"
	...
	// Optionally prune remote miner branches
	if minerPruneRemote {
		fmt.Println()
		fmt.Println("Pruning remote miner branches...")

		defaultBranch := repoGit.RemoteDefaultBranch()
		remoteRefs, lsErr := repoGit.ListPushRemoteRefsWithHashes("origin", "refs/heads/miner/")
		if lsErr != nil {
		...
		if remotePruned == 0 {
			fmt.Println("No stale remote miner branches found.")
		} else {
			verb := "Pruned"
			if minerPruneDryRun {
				verb = "Would prune"
...
// runMinerPoolInit creates a persistent miner pool for a rig.
// Creates N miners with identities and worktrees in IDLE state.
// Existing miners are preserved — only new ones are created.
func runMinerPoolInit(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	...
	if cfgErr == nil && rigCfg.MinerPoolSize > 0 {
		poolSize = rigCfg.MinerPoolSize
	}
	if minerPoolInitSize > 0 {
		poolSize = minerPoolInitSize
	}

	// Determine names: rig config > name pool theme
	var fixedNames []string
	if cfgErr == nil && len(rigCfg.MinerNames) > 0 {
		fixedNames = rigCfg.MinerNames
	}

	// List existing miners to avoid recreating them
	existing, err := mgr.List()
	if err != nil {
		return fmt.Errorf("listing existing miners: %w", err)
	}
	...
	fmt.Printf("Initializing persistent miner pool for %s (target size: %d)\n", rigName, poolSize)
	if len(existing) > 0 {
		fmt.Printf("  Existing miners: %d\n", len(existing))
	}
	...
	if len(fixedNames) > 0 {
		...
		for len(namesToCreate)+len(existingNames) < poolSize {
			...
			if allocErr != nil {
				return fmt.Errorf("allocating miner name: %w", allocErr)
			}
			if !existingNames[name] {
				namesToCreate = append(namesToCreate, name)
			}
	...
	if len(namesToCreate) == 0 {
		fmt.Printf("\n%s Pool already at target size (%d miners).\n", style.Bold.Render("✓"), len(existing))
		return nil
	}

	if minerPoolInitDryRun {
		fmt.Printf("\nWould create %d miner(s):\n", len(namesToCreate))
	...
	for _, name := range namesToCreate {
		...
		// Set agent state to idle (miner was created without work).
		...
		// SetAgentStateWithRetry's other call site in miner_spawn.go.
...
// existingNamesList extracts miner names from a slice of Miner pointers.
func existingNamesList(miners []*miner.Miner) []string {
	names := make([]string, len(miners))
