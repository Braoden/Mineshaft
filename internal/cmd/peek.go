...
This is the ergonomic alias for 'ms session capture'. Use it to check
...
The nudge/peek pair provides the canonical interface for agent sessions:
  ms nudge - send messages TO a session (reliable delivery)
  ms peek  - read output FROM a session (capture-pane wrapper)
...
Examples:
  ms peek greenplace/furiosa         # Miner: last 100 lines (default)
  ms peek greenplace/furiosa 50      # Miner: last 50 lines
  ms peek beads/crew/dave            # Crew: last 100 lines
  ms peek beads/crew/dave -n 200     # Crew: last 200 lines
  ms peek overseer                      # Overseer: last 100 lines
  ms peek supervisor -n 50               # Supervisor: last 50 lines`,
...
func runPeek(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if !strings.Contains(address, "/") {
			return fmt.Errorf("not in a rig directory. Use full address format: ms peek <rig>/<miner>")
		}
		return err
	}
	...
	// e.g., "beads/crew/dave" -> session name "ms-beads-crew-dave"
