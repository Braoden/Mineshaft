...
case "$*" in
    ...
    if [ "$PWD" != "%s" ]; then
      ...
      exit 1
	    ...
	    echo '[{"id":"hq-town","title":"Town task","status":"open","issue_type":"task","assignee":"overseer","labels":["kind/bug"]}]'
	    ;;
...
`, logPath, expectedTownWD, expectedTownWD, expectedRigWD, expectedRigWD)
	...
	if got["hq-town"] == nil || got["hq-town"].Status != "open" || got["hq-town"].Assignee != "overseer" {
...
func TestCloseMinecartIfComplete_UnknownBlocksAutoClose(t *testing.T) {
	...
	// No bd stub — closeMinecartIfComplete does not shell out when the minecart
	...
	out, err := captureMinecartStdoutErr(t, func() error {
		ready, err := closeMinecartIfComplete(townBeads, "hq-cv-unreach", "Mixed", tracked, false)
		if ready {
			t.Fatalf("closeMinecartIfComplete reported ready with unknown tracked status")
		}
		return err
	})
	if err != nil {
		t.Fatalf("closeMinecartIfComplete: %v", err)
	}
