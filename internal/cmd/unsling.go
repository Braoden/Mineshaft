...
Examples:
  ms unsling                        # Clear my hook (whatever's there)
  ms unsling ms-abc                 # Only unsling if ms-abc is hooked
  ms unsling greenplace/joe            # Clear joe's hook
  ms unsling ms-abc greenplace/joe     # Unsling ms-abc from joe

The bead's status changes from 'hooked' back to 'open'.

Related commands:
  ms sling <bead>    # Hook + start (inverse of unsling)
  ms hook <bead>     # Hook without starting
  ms hook      # See what's on your hook`,
...
func runUnslingWith(cmd *cobra.Command, args []string, dryRun, force bool) error {
	...
	// Without this, ms unsling can't find or clear these beads, even though
	// ms mol status / ms hook (display) can see them. (ms-dtq7)
	...
	if hookedBeadID == "" {
		...
		// wasn't updated). Clean them up so ms hook and ms unsling stay consistent.
...
// bead's status wasn't updated back to "open". Without this, ms hook shows the
// stale hook (via fallback query) but ms unsling says "Nothing on your hook".
...
func cleanStaleHookedBeads(cmd *cobra.Command, b *beads.Beads, agentID, targetBeadID, townRoot, beadsPath string, dryRun bool) bool {
	...
	// Also search town-level beads for rig-level agents (ms-dtq7).
