...
When run by an agent (MS_ROLE set), this command:
...
Examples:
  ms commit -m "Fix bug"              # Commit as current agent
  ms commit -am "Quick fix"           # Stage all and commit
  ms commit -- --amend                # Amend last commit

Identity mapping:
  Agent: mineshaft/crew/jack  →  Name: mineshaft/crew/jack
                                Email: mineshaft.crew.jack@mineshaft.local

When run without MS_ROLE (human), passes through to git commit with no changes.`,
	RunE:               runCommit,
