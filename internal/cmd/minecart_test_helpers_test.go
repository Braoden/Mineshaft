...
// Minecart adds a minecart bead to the DAG.
func (d *testDAG) Minecart(id, title string) *testDAG {
	d.t.Helper()
	d.beads[id] = &testBead{
		ID:     id,
		Title:  title,
...
// TrackedBy adds a "tracks" dependency: minecartID tracks the last-added bead.
// Models: minecart depends on the bead via "tracks" (matching production
// `bd dep add minecartID beadID --type=tracks`).
func (d *testDAG) TrackedBy(minecartID string) *testDAG {
	d.t.Helper()
	if d.last == "" {
...
func (d *testDAG) BdStubScript() string {
	...
	// For "down" direction (minecart → tracked beads): match on issue_id = '<minecartID>'
	for id, b := range d.beads {
		if b.Type == "minecart" {
	...
	// For "up" direction (bead → tracking minecarts): match typed target columns.
	...
	// --- handle: dep list --direction=down (tracked beads for minecarts) ---
...
// trackedBeadsJSONFor returns the JSON array for `bd dep list <minecartID>
...
func (d *testDAG) trackedBeadsJSONFor(minecartID string) string {
	type idOnly struct {
		ID string `json:"id"`
	}
	...
	var out []idOnly
	for _, dep := range d.deps {
		// tracks deps: IssueID is the minecart, DependsOnID is the tracked bead.
		if dep.Type == "tracks" && dep.IssueID == minecartID {
...
// FROM dependencies WHERE issue_id = '<minecartID>' AND type = 'tracks'" --json`.
// Returns [{"depends_on_id":"<id>"},...] for each tracked bead.
func (d *testDAG) trackedBeadsSQLJSONFor(minecartID string) string {
	type sqlRow struct {
...
// Returns [{"issue_id":"<minecartID>"},...] for each minecart tracking this bead.
...
// minecartListJSON returns the JSON array for minecart list queries.
// Returns all minecart beads with their ID and status.
func (d *testDAG) minecartListJSON() string {
	type minecartEntry struct {
		ID     string `json:"id"`
		Status string `json:"status"`
	}

	var out []minecartEntry
	for _, b := range d.beads {
		if b.Type == "minecart" {
			status := b.Status
			if status == "" {
			...
			out = append(out, minecartEntry{ID: b.ID, Status: status})
		}
	}
	if out == nil {
		return "[]"
