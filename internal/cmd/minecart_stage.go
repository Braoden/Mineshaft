...
var minecartStageTitle string

// minecartStageNoValidate disables automatic validation bead creation for epic input.
var minecartStageNoValidate bool

func init() {
	minecartStageCmd.Flags().BoolVar(&minecartStageJSON, "json", false, "Output machine-readable JSON")
	minecartStageCmd.Flags().BoolVar(&minecartStageLaunch, "launch", false, "Launch the minecart immediately after staging (transition to open)")
	minecartStageCmd.Flags().StringVar(&minecartStageTitle, "title", "", "Human-readable title for the minecart (default: derived from epic title or auto-generated)")
	minecartStageCmd.Flags().BoolVar(&minecartStageNoValidate, "no-validate", false, "Skip automatic validation bead creation (epic input only)")
	minecartStageCmd.SetFlagErrorFunc(minecartStageFlagError)
}
...
type StageResult struct {
	...
	Restaged         bool            `json:"restaged"`                     // true if an existing minecart was updated in place
...
// beadTypes maps bead ID → type string (e.g. "epic", "task", "minecart").
// Returns error if types are mixed (e.g., epic + task) or if multiple
// epics/minecarts are provided (only one is allowed).
...
var minecartStageCmd = &cobra.Command{
	Use:   "stage <epic-id | task-id... | minecart-id>",
	Short: "Stage a minecart: analyze dependencies, compute waves, create staged minecart",
...
func minecartStageFlagError(cmd *cobra.Command, err error) error {
	if !minecartStageFlagEnabled(cmd) {
		return err
	}
	if cmd != nil {
...
func minecartStageFlagEnabled(cmd *cobra.Command) bool {
	if minecartStageJSON {
		return true
	}
	if cmd == nil {
		return argsRequestMinecartStageJSON(os.Args[1:])
	}
	jsonFlag, err := cmd.Flags().GetBool("json")
	return err == nil && jsonFlag || argsRequestMinecartStageJSON(os.Args[1:])
}

func argsRequestMinecartStageJSON(args []string) bool {
	for _, arg := range args {
		if arg == "--json" {
			return true
		}
...
func runMinecartStage(cmd *cobra.Command, args []string) error {
	if cmd != nil {
		cmd.SilenceErrors = minecartStageJSON
		cmd.SilenceUsage = minecartStageJSON
	}
	...
	// If input is a minecart that is already staged, we update in place.
	isRestage := false
	restageMinecartID := ""
	if input.Kind == StageInputMinecart {
		minecartResult := beadResults[input.IDs[0]]
		if strings.HasPrefix(minecartResult.Status, "staged_") {
			isRestage = true
			restageMinecartID = input.IDs[0]
		}
	}
	...
	// Step 5b: Detect overlapping minecarts (epic or task-list input only).
	// When the user stages from an epic (or task list), check if an existing
	// minecart already tracks these beads. This prevents duplicate minecarts.
	if !isRestage && input.Kind != StageInputMinecart {
		slingableIDs := dagSlingableIDs(dag)
		overlaps, err := findOverlappingMinecarts(slingableIDs)
		if err != nil {
			err = fmt.Errorf("checking for overlapping minecarts: %w", err)
			if minecartStageJSON {
				return emitStageJSONError("overlap", slingableIDs, err, dag, input)
			}
			return err
		}
		autoRestage, autoMinecartID, err := handleOverlappingMinecarts(overlaps)
		if err != nil {
			if minecartStageJSON {
				return emitStageJSONError("overlap", slingableIDs, err, dag, input)
			}
		...
		if autoRestage {
			isRestage = true
			restageMinecartID = autoMinecartID
			if !minecartStageJSON {
				fmt.Printf("Re-staging existing minecart %s\n", autoMinecartID)
			}
		}
	}
	...
	if minecartStageJSON {
		return runMinecartStageJSON(dag, input, errs, warns, status, isRestage, restageMinecartID)
	}

	// Step 10: If errors, render and return.
	if len(errs) > 0 {
		fmt.Fprint(os.Stderr, renderErrors(errs))
		return fmt.Errorf("minecart staging failed: %d error(s) found", len(errs))
	}
	...
	if input.Kind == StageInputEpic && !minecartStageNoValidate {
		epicID := input.IDs[0]
		...
		if validationID != "" && !minecartStageJSON {
	...
	// Step 14b: Resolve minecart title.
	title := resolveMinecartTitle(minecartStageTitle, input, beadResults)

	// Step 15: Create or update the staged minecart.
	var minecartID string
	if isRestage {
		// Re-stage: update existing minecart in place.
		if err := updateStagedMinecart(restageMinecartID, dag, waves, status, title); err != nil {
			return err
		}
		minecartID = restageMinecartID
		fmt.Printf("Minecart updated: %s (status: %s)\n", restageMinecartID, status)
	} else {
		// First stage: create a new minecart.
		var err error
		minecartID, err = createStagedMinecart(dag, waves, status, title)
		if err != nil {
			return err
		}
		fmt.Printf("Minecart created: %s (status: %s)\n", minecartID, status)
	}

	// Step 16: If --launch flag is set, transition to open immediately.
	if minecartStageLaunch {
		if err := transitionMinecartToOpen(minecartID, minecartLaunchForce); err != nil {
			return err
		}
		fmt.Printf("Minecart launched: %s (status: open)\n", minecartID)
	}

	return nil
}

// runMinecartStageJSON handles the --json output path for minecart staging.
...
func runMinecartStageJSON(dag *MinecartDAG, input *StageInput, errs, warns []StagingFinding, status string, isRestage bool, restageMinecartID string) error {
	result := StageResult{
	...
	if len(errs) > 0 {
		// Errors: no minecart created, status is "error", no waves.
		result.Status = "error"
		result.Waves = []WaveJSON{}

		return emitStageJSONResult(result, fmt.Errorf("minecart staging failed: %d error(s) found", len(errs)))
	}

	// No errors: compute waves and create/update minecart.
	waves, gated, err := computeWaves(dag)
	if err != nil {
	...
	// Resolve minecart title for JSON path.
	title := resolveMinecartTitle(minecartStageTitle, input, nil)

	if isRestage {
		if err := updateStagedMinecart(restageMinecartID, dag, waves, status, title); err != nil {
			return emitStageJSONError("minecart", []string{restageMinecartID}, err, dag, input)
		}
		result.MinecartID = restageMinecartID
		result.Restaged = true
	} else {
		minecartID, err := createStagedMinecart(dag, waves, status, title)
		if err != nil {
			return emitStageJSONError("minecart", nil, err, dag, input)
		}
		result.MinecartID = minecartID
	}
...
// ---------------------------------------------------------------------------
// Overlapping minecart detection (prevent duplicate minecart creation)
// ---------------------------------------------------------------------------

// overlappingMinecart describes a minecart that tracks beads overlapping with a
// staging request.
type overlappingMinecart struct {
	ID           string // minecart bead ID (e.g., "hq-cv-abc")
	Status       string // minecart status (e.g., "staged_ready", "open")
	OverlapCount int    // number of slingable IDs also tracked by this minecart
}

// dagSlingableIDs returns sorted slingable bead IDs from a DAG.
func dagSlingableIDs(dag *MinecartDAG) []string {
	var ids []string
...
// minecartTrackedBeadIDs returns the set of bead IDs tracked by a minecart.
...
// database (e.g., ds-* issues tracked by an hq-cv-* minecart). See GH #2624.
func minecartTrackedBeadIDs(townBeads, minecartID string) (map[string]bool, error) {
	trackedIDs, err := bdDepListRawIDs(townBeads, minecartID, "down", "tracks")
	if err != nil {
		return nil, fmt.Errorf("tracked deps for %s: %w", minecartID, err)
	}
...
// findOverlappingMinecarts lists all staged and open minecarts and checks which
// ones track beads that overlap with the given slingable IDs.
// Only staged_* and open minecarts are considered; closed minecarts are skipped.
func findOverlappingMinecarts(slingableIDs []string) ([]overlappingMinecart, error) {
	townBeads, err := getTownBeadsDir()
	if err != nil {
		return nil, err
	}

	minecarts, err := listMinecartIssues(townBeads, "", true)
	if err != nil {
	...
	var overlaps []overlappingMinecart
	for _, c := range minecarts {
		status := normalizeMinecartStatus(c.Status)
		// Only consider staged and open minecarts.
		if !isStagedStatus(status) && status != minecartStatusOpen {
			continue
		}

		trackedIDs, err := minecartTrackedBeadIDs(townBeads, c.ID)
		if err != nil {
			// Skip minecarts whose tracked beads can't be resolved.
			continue
		}
		...
		if overlapCount > 0 {
			overlaps = append(overlaps, overlappingMinecart{
				ID:           c.ID,
				Status:       status,
...
// handleOverlappingMinecarts decides what to do when existing minecarts overlap
// with the beads being staged.
//
// Returns:
//   - (true, minecartID, nil): auto re-stage the given minecart
//   - (false, "", nil): no overlaps, proceed with fresh creation
//   - (false, "", error): cannot proceed (open minecart or ambiguous)
func handleOverlappingMinecarts(overlaps []overlappingMinecart) (bool, string, error) {
	if len(overlaps) == 0 {
		return false, "", nil
	}

	// Separate by status.
	var staged []overlappingMinecart
	var open []overlappingMinecart
	for _, o := range overlaps {
		if isStagedStatus(o.Status) {
			staged = append(staged, o)
		} else if o.Status == minecartStatusOpen {
			open = append(open, o)
		}
	}

	// Open minecart overlap → error.
	if len(open) > 0 {
		o := open[0]
...
// resolveMinecartTitle determines the minecart title.
// Priority: explicit --title flag > derived from epic title > auto-generated (empty).
func resolveMinecartTitle(flagTitle string, input *StageInput, beadResults map[string]*bdShowResult) string {
	if flagTitle != "" {
		return flagTitle
	}
...
// createStagedMinecart creates a minecart with the given staged status.
...
func createStagedMinecart(dag *MinecartDAG, waves []Wave, status string, title string) (string, error) {
	...
	description := fmt.Sprintf("Staged minecart: %d tasks, %d waves. Staged at %s",
		taskCount, len(waves), time.Now().UTC().Format(time.RFC3339))

	// Create the minecart via bd create in town beads, then set status via bd update.
	createArgs := []string{
		"create",
		"--type=task",
		"--id=" + minecartID,
	...
	if out, err := BdCmd(createArgs...).Dir(townBeads).WithAutoCommit().CombinedOutput(); err != nil {
		return "", fmt.Errorf("bd create minecart: %w\noutput: %s", err, out)
	}
	...
	if out, err := BdCmd("update", minecartID, "--status="+status).
		Dir(townBeads).StripBeadsDir().WithAutoCommit().
		CombinedOutput(); err != nil {
		return "", fmt.Errorf("bd update minecart status: %w\noutput: %s", err, out)
	}

	// Track each slingable bead via the typed dependency helper.
	for _, beadID := range slingableIDs {
		if err := addTrackingRelationFn(townBeads, minecartID, beadID); err != nil {
			printStageWarning("  Warning: could not track %s in minecart: %v\n", beadID, err)
		}
	}
...
// updateStagedMinecart updates an existing staged minecart in place.
...
// Minecart beads live in the town HQ beads database, so all bd commands
// run against getTownBeadsDir().
func updateStagedMinecart(existingMinecartID string, dag *MinecartDAG, waves []Wave, status string, title string) error {
	// Minecarts live in the town HQ beads database.
	townBeads, err := getTownBeadsDir()
	if err != nil {
		return err
	...
	currentIDs, err := minecartTrackedBeadIDs(townBeads, existingMinecartID)
	if err != nil {
		return fmt.Errorf("reading tracked beads for %s: %w", existingMinecartID, err)
	}

	// Add new beads not currently tracked.
	for _, id := range desiredIDs {
		if !currentIDs[id] {
			if err := addTrackingRelationFn(townBeads, existingMinecartID, id); err != nil {
				printStageWarning("  Warning: could not track %s in minecart: %v\n", id, err)
			}
		}
	}
	...
	for id := range currentIDs {
		if !desiredSet[id] {
			if err := removeTrackingRelationFn(townBeads, existingMinecartID, id); err != nil {
				printStageWarning("  Warning: could not untrack %s from minecart: %v\n", id, err)
			}
		}
	}

	// Update status.
	if out, err := BdCmd("update", existingMinecartID, "--status="+status).
		Dir(townBeads).WithAutoCommit().
		CombinedOutput(); err != nil {
		return fmt.Errorf("bd update %s --status: %w\noutput: %s", existingMinecartID, err, out)
	}

	// Update title if provided.
	if title != "" {
		if out, err := BdCmd("update", existingMinecartID, "--title="+title).
			Dir(townBeads).WithAutoCommit().
			CombinedOutput(); err != nil {
	...
	description := fmt.Sprintf("Staged minecart: %d tasks, %d waves. Re-staged at %s",
		len(desiredIDs), len(waves), time.Now().UTC().Format(time.RFC3339))
	if out, err := BdCmd("update", existingMinecartID, "--description="+description).
		Dir(townBeads).WithAutoCommit().
		CombinedOutput(); err != nil {
...
// MinecartDAG represents an in-memory dependency graph for minecart staging.
type MinecartDAG struct {
	Nodes map[string]*MinecartDAGNode
}

// MinecartDAGNode represents a single bead in the DAG.
type MinecartDAGNode struct {
	ID        string
	Title     string
...
func detectCycles(dag *MinecartDAG) []string {
	const (
		white = 0
		gray  = 1
		black = 2
...
// isSlingableType delegates to the canonical minecart.IsSlingableType, which
// handles empty types (legacy beads that default to "task").
func isSlingableType(beadType string) bool {
	return minecart.IsSlingableType(beadType)
}
...
func computeWaves(dag *MinecartDAG) ([]Wave, []GatedTask, error) {
	// Step 1: Filter to slingable types only.
	slingable := make(map[string]*MinecartDAGNode)
...
func appendValidationWave(dag *MinecartDAG, waves []Wave, epicID string) ([]Wave, string, error) {
	townBeads, err := getTownBeadsDir()
	if err != nil {
		return waves, "", err
	}
	...
	dag.Nodes[validationID] = &MinecartDAGNode{
		ID:        validationID,
		Title:     "Validate: PRD success criteria",
		Type:      "task",
		Status:    "open",
...
// buildMinecartDAG constructs a MinecartDAG from raw bead and dependency data.
// Edge classification:
...
func buildMinecartDAG(beads []BeadInfo, deps []DepInfo) *MinecartDAG {
	dag := &MinecartDAG{Nodes: make(map[string]*MinecartDAGNode)}

	// Create nodes from beads.
	for _, b := range beads {
		dag.Nodes[b.ID] = &MinecartDAGNode{
			ID:     b.ID,
			Title:  b.Title,
			Type:   b.Type,
			Status: b.Status,
...
// StagingFinding represents an error or warning found during minecart staging analysis.
...
func detectErrors(dag *MinecartDAG) []StagingFinding {
...
// chooseStatus determines the minecart status based on analysis results.
// Returns "" if errors found (no minecart should be created).
...
func renderGatedTasks(gated []GatedTask, dag *MinecartDAG) string {
	var buf strings.Builder
...
func buildGatedJSON(gated []GatedTask, dag *MinecartDAG) []GatedTaskJSON {
	if len(gated) == 0 {
		return nil
	}
...
// For minecart input: reads tracked beads via typed dependency target columns.
...
func collectBeads(input *StageInput) ([]BeadInfo, []DepInfo, error) {
	...
	case StageInputMinecart:
		return collectMinecartBeads(input.IDs[0])
	}
...
// collectMinecartBeads reads tracked beads from an existing minecart.
func collectMinecartBeads(minecartID string) ([]BeadInfo, []DepInfo, error) {
	// 1. Validate minecart exists.
	_, err := bdShow(minecartID)
	if err != nil {
		return nil, nil, fmt.Errorf("minecart %s: %w", minecartID, err)
	}

	// 2. Read tracked IDs using the same filtered dep-list path used by status
	// and staged-minecart reconciliation. This handles id-only dep rows and
	// external:<rig>:<id> wrappers.
	townBeads, err := getTownBeadsDir()
	if err != nil {
		return nil, nil, err
	}
	trackedSet, err := minecartTrackedBeadIDs(townBeads, minecartID)
	if err != nil {
		return nil, nil, fmt.Errorf("deps for minecart %s: %w", minecartID, err)
	}
	...
	if len(trackedIDs) == 0 {
		return nil, nil, fmt.Errorf("minecart %s tracks no beads", minecartID)
	}
...
func renderDAGTree(dag *MinecartDAG, input *StageInput) string {
	...
	default:
		// Flat list for task-list and minecart input.
...
func renderTreeNode(dag *MinecartDAG, nodeID string, prefix string, isLast bool, buf *strings.Builder) {
...
func formatNodeLine(node *MinecartDAGNode) string {
...
func sortedChildren(dag *MinecartDAG, nodeID string) []string {
	node := dag.Nodes[nodeID]
...
func sortedNodeIDs(dag *MinecartDAG) []string {
	ids := make([]string, 0, len(dag.Nodes))
	for id := range dag.Nodes {
		ids = append(ids, id)
	}
...
func detectWarnings(dag *MinecartDAG, input *StageInput) []StagingFinding {
	var findings []StagingFinding
...
// Only applies to epic input — task-list and minecart input never warn about
// orphans because isolation is expected.
func detectOrphans(dag *MinecartDAG, input *StageInput) []StagingFinding {
...
func detectCrossRig(dag *MinecartDAG) []StagingFinding {
...
func estimateCapacity(dag *MinecartDAG) []StagingFinding {
...
func detectMissingBranches(dag *MinecartDAG) []StagingFinding {
	var findings []StagingFinding
	for _, node := range dag.Nodes {
		if node.Type != "epic" {
			continue
...
func emitStageJSONError(category string, beadIDs []string, err error, dag *MinecartDAG, input *StageInput) error {
	if err == nil {
		err = fmt.Errorf("minecart staging failed")
	}
	if category == "" {
		category = "error"
	}
...
func buildWavesJSON(waves []Wave, dag *MinecartDAG) []WaveJSON {
...
// For task-list or minecart input, returns a flat list of nodes.
func buildTreeJSON(dag *MinecartDAG, input *StageInput) []TreeNodeJSON {
	switch input.Kind {
	case StageInputEpic:
		if len(input.IDs) == 0 {
			return []TreeNodeJSON{}
	...
	default:
		// Flat list for task-list and minecart input, sorted by ID.
		ids := sortedNodeIDs(dag)
...
func buildTreeNodeJSON(dag *MinecartDAG, node *MinecartDAGNode) TreeNodeJSON {
	tn := TreeNodeJSON{
		ID:     node.ID,
		Title:  node.Title,
		Type:   node.Type,
