...
func TestVerifyBranch(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fields:      &beads.MRFields{Branch: "miner/Nux/ms-abc"},
			wantMissing: false,
			wantErr:     false,
		},
		{
		...
		},
		{
			...
			verify: true,
			client: &mockBranchVerifier{
				localBranches: map[string]bool{"miner/Nux/ms-abc": true},
			},
			fields:      &beads.MRFields{Branch: "miner/Nux/ms-abc"},
			wantMissing: false,
			wantErr:     false,
		},
		{
			...
			client: &mockBranchVerifier{
				localBranches:  map[string]bool{},
				remoteBranches: map[string]bool{"origin/miner/Nux/ms-abc": true},
			},
			fields:      &beads.MRFields{Branch: "miner/Nux/ms-abc"},
			wantMissing: false,
			wantErr:     false,
