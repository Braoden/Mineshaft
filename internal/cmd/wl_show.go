...
Unlike 'ms wl browse' which truncates titles and omits descriptions,
'ms wl show' displays every field of the item.

The local wl-commons database is queried directly (kept in sync by ms wl sync).

EXAMPLES:
  ms wl show w-abc123             # Display all fields
  ms wl show w-abc123 --json      # JSON output`,
