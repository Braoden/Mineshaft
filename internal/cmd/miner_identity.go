...
var (
	minerIdentityListJSON    bool
	minerIdentityShowJSON    bool
	minerIdentityRemoveForce bool
)

var minerIdentityCmd = &cobra.Command{
	Use:     "identity",
	Aliases: []string{"id"},
	Short:   "Manage miner identities",
	Long: `Manage miner identity beads in rigs.

Identity beads track miner metadata, CV history, and lifecycle state.
Use subcommands to create, list, show, rename, or remove identities.`,
	RunE: requireSubcommand,
}

var minerIdentityAddCmd = &cobra.Command{
	Use:   "add <rig> [name]",
	Short: "Create an identity bead for a miner",
	Long: `Create an identity bead for a miner in a rig.
...
var minerIdentityListCmd = &cobra.Command{
	Use:   "list <rig>",
	Short: "List miner identity beads in a rig",
	Long: `List all miner identity beads in a rig.
...
var minerIdentityShowCmd = &cobra.Command{
	Use:   "show <rig> <name>",
	Short: "Show miner identity with CV summary",
	Long: `Show detailed identity information for a miner including work history.
...
var minerIdentityRenameCmd = &cobra.Command{
	Use:   "rename <rig> <old-name> <new-name>",
	Short: "Rename a miner identity (preserves CV)",
	Long: `Rename a miner identity bead, preserving CV history.
...
var minerIdentityRemoveCmd = &cobra.Command{
	Use:   "remove <rig> <name>",
	Short: "Remove a miner identity",
	Long: `Remove a miner identity bead.
...
func init() {
	// List flags
	minerIdentityListCmd.Flags().BoolVar(&minerIdentityListJSON, "json", false, "Output as JSON")

	// Show flags
	minerIdentityShowCmd.Flags().BoolVar(&minerIdentityShowJSON, "json", false, "Output as JSON")

	// Remove flags
	minerIdentityRemoveCmd.Flags().BoolVarP(&minerIdentityRemoveForce, "force", "f", false, "Force removal, bypassing safety checks")

	// Add subcommands to identity
	minerIdentityCmd.AddCommand(minerIdentityAddCmd)
	minerIdentityCmd.AddCommand(minerIdentityListCmd)
	minerIdentityCmd.AddCommand(minerIdentityShowCmd)
	minerIdentityCmd.AddCommand(minerIdentityRenameCmd)
	minerIdentityCmd.AddCommand(minerIdentityRemoveCmd)

	// Add identity to miner command
	minerCmd.AddCommand(minerIdentityCmd)
}
...
// CVSummary represents the CV/work history summary for a miner.
...
func runMinerIdentityAdd(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	...
	if minerName == "" {
		...
		minerName, err = mgr.AllocateName()
		if err != nil {
			return fmt.Errorf("generating miner name: %w", err)
		}
	...
	bd := beads.New(r.Path)
	beadID := minerBeadIDForRig(r, rigName, minerName)
	...
	title := fmt.Sprintf("Miner %s in %s", minerName, rigName)
...
func runMinerIdentityList(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	...
	// Filter for miner beads in this rig
	...
	for id, issue := range agentBeads {
		// Parse the bead ID to check if it's a miner for this rig
		beadRig, role, name, ok := beads.ParseAgentBeadID(id)
		if !ok || role != constants.RoleMiner || beadRig != rigName {
			continue
		}
		...
		mgr := miner.NewManager(r, nil, t)
		if p, err := mgr.Get(name); err == nil && p != nil {
			worktreeExists = true
		}

		// Check if session is running
		sessionRunning, _ := minerMgr.IsRunning(name)
	...
	if len(identities) == 0 {
		fmt.Printf("No miner identities found in %s.\n", rigName)
		return nil
	}

	fmt.Printf("%s\n\n", style.Bold.Render(fmt.Sprintf("Miner Identities in %s", rigName)))
...
func runMinerIdentityShow(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	minerName := args[1]
	...
	if p, err := mgr.Get(minerName); err == nil && p != nil {
		worktreeExists = true
		clonePath = p.ClonePath
	}
	sessionRunning, _ := minerMgr.IsRunning(minerName)

	// Build CV summary with enhanced analytics
	cv := buildCVSummary(r.Path, rigName, minerName, beadID, clonePath)

	// JSON output - include both identity details and CV
	if minerIdentityShowJSON {
		output := struct {
			IdentityInfo
			Title     string     `json:"title"`
			CreatedAt string     `json:"created_at,omitempty"`
	...
	// Human-readable output
	fmt.Printf("\n%s %s/%s\n", style.Bold.Render("Identity:"), rigName, minerName)
...
func runMinerIdentityRename(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	oldName := args[1]
	newName := args[2]
	...
	bd := beads.New(r.Path)
	oldBeadID := minerBeadIDForRig(r, rigName, oldName)
	newBeadID := minerBeadIDForRig(r, rigName, newName)
	...
	t := tmux.NewTmux()
	minerMgr := miner.NewSessionManager(t, r)
	running, _ := minerMgr.IsRunning(oldName)
	if running {
		return fmt.Errorf("cannot rename: miner session %s is running", oldName)
	}
	...
	newTitle := fmt.Sprintf("Miner %s in %s", newName, rigName)
...
func runMinerIdentityRemove(cmd *cobra.Command, args []string) error {
	rigName := args[0]
	minerName := args[1]
	...
	if !minerIdentityRemoveForce {
		var reasons []string

		// Check for active session
		t := tmux.NewTmux()
		minerMgr := miner.NewSessionManager(t, r)
		running, _ := minerMgr.IsRunning(minerName)
		if running {
		...
		// Warn if CV exists
		assignee := fmt.Sprintf("%s/%s", rigName, minerName)
		...
		if cvCount > 0 {
			fmt.Printf("%s Warning: This miner has %d completed work item(s) in CV.\n",
...
// buildCVSummary constructs the CV summary for a miner.
// Returns a partial CV on errors rather than failing - CV data is best-effort.
func buildCVSummary(rigPath, rigName, minerName, identityBeadID, clonePath string) *CVSummary {
	cv := &CVSummary{
	...
	cv.Sessions = countMinerSessions(rigPath, minerName)

	// Query completed issues assigned to this miner
...
// countMinerSessions counts the number of sessions from checkpoint files.
func countMinerSessions(rigPath, minerName string) int {
	// Look for checkpoint files in the miner's directory
	checkpointDir := filepath.Join(rigPath, "miners", minerName, ".checkpoints")
	...
	// Count checkpoint files that contain this miner's name
	count := 0
	for _, entry := range entries {
		if !entry.IsDir() && strings.Contains(entry.Name(), minerName) {
			count++
		}
	}

	// If no checkpoint files found, return at least 1 if miner exists
	if count == 0 {
		return 1
	}
	return count
