...
// spawnMinerForSling is a seam for tests. Production uses SpawnMinerForSling.
var spawnMinerForSling = SpawnMinerForSling
...
// buildAgentIdentity: town-level agents (overseer, supervisor) get a trailing slash.
...
func canonicalAssigneeAddress(identity *session.AgentIdentity) string {
	...
	case session.RoleOverseer, session.RoleSupervisor:
...
func resolveSelfTarget() (agentID string, pane string, hookRoot string, err error) {
	...
	case RoleSupervisor:
		agentID = "supervisor/"
	case RoleBoot:
		agentID = "supervisor/boot"
	case RoleWitness:
		agentID = fmt.Sprintf("%s/witness", roleInfo.Rig)
	case RoleRefinery:
		agentID = fmt.Sprintf("%s/refinery", roleInfo.Rig)
	case RoleMiner:
		agentID = fmt.Sprintf("%s/miners/%s", roleInfo.Rig, roleInfo.Miner)
	case RoleCrew:
		agentID = fmt.Sprintf("%s/crew/%s", roleInfo.Rig, roleInfo.Miner)
	case RoleDog:
		agentID = fmt.Sprintf("supervisor/dogs/%s", roleInfo.Miner)
	default:
...
type ResolveTargetOptions struct {
	...
	Agent                string
	NoBoot               bool
	HookBead             string // Bead ID to set atomically during miner spawn (empty = skip)
	...
	SkipMinerAdmission bool   // Caller already holds a capacity reservation
}
...
type ResolvedTarget struct {
	...
	NewMinerInfo    *SpawnedMinerInfo
	IsSelfSling       bool
}

// resolveTarget resolves a target specification to agent, pane, and working directory.
// Handles: "." or empty (self), dog targets, rig targets (auto-spawn miner),
// existing agents (with dead miner fallback).
func resolveTarget(target string, opts ResolveTargetOptions) (*ResolvedTarget, error) {
	...
	if dogName, isDog := IsDogTarget(target); isDog {
		if opts.DryRun {
			if dogName == "" {
				...
				result.Agent = "supervisor/dogs/<idle>"
			} else {
				fmt.Printf("Would dispatch to dog '%s'\n", dogName)
				result.Agent = fmt.Sprintf("supervisor/dogs/%s", dogName)
			}
	...
	if rigName, isRig := IsRigName(target); isRig {
		...
		if opts.BeadID != "" && !opts.Force {
			if err := checkCrossRigGuard(opts.BeadID, rigName+"/miners/_", opts.TownRoot); err != nil {
				return nil, err
			}
		}
		if opts.BeadID != "" {
		...
		if opts.DryRun {
			fmt.Printf("Would spawn fresh miner in rig '%s'\n", rigName)
			result.Agent = fmt.Sprintf("%s/miners/<new>", rigName)
			result.Pane = "<new-pane>"
			return result, nil
		}
		fmt.Printf("Target is rig '%s', spawning fresh miner...\n", rigName)
		spawnOpts := SlingSpawnOptions{
			...
			SkipAdmission: opts.SkipMinerAdmission,
		}
		spawnInfo, err := spawnMinerForSling(rigName, spawnOpts)
		if err != nil {
			return nil, fmt.Errorf("spawning miner: %w", err)
		}
		result.Agent = spawnInfo.AgentID()
		result.NewMinerInfo = spawnInfo
	...
	if err != nil {
		if rigName, ok := missingMinerTargetRig(target, opts.Create, opts.TownRoot); ok {
			...
			fmt.Printf("Target miner has no active session, spawning fresh miner in rig '%s'...\n", rigName)
			spawnOpts := SlingSpawnOptions{
				TownRoot:      opts.TownRoot,
				Force:         opts.Force,
				Account:       opts.Account,
			...
			spawnInfo, spawnErr := spawnMinerForSling(rigName, spawnOpts)
			if spawnErr != nil {
				return nil, fmt.Errorf("spawning miner to replace dead miner: %w", spawnErr)
			}
			result.Agent = spawnInfo.AgentID()
			result.NewMinerInfo = spawnInfo
			result.WorkDir = spawnInfo.ClonePath
	...
	if opts.BeadID != "" && isMinerTarget(agentID) {
	...
	result.Pane = pane
	result.WorkDir = workDir
	// Detect self-sling by pane: a named target (e.g. "supervisor") that resolves to
...
func missingMinerTargetRig(target string, allowShorthand bool, townRoot string) (string, bool) {
	if isMinerTarget(target) {
