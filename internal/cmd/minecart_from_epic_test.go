...
func TestMinecartCreate_FromEpicFlagExists(t *testing.T) {
	flag := minecartCreateCmd.Flags().Lookup("from-epic")
	if flag == nil {
		t.Fatal("minecartCreateCmd should have --from-epic flag")
	}
	if flag.DefValue != "" {
...
func TestMinecartCreate_InvalidMergeFlag(t *testing.T) {
	tests := []struct {
		value   string
		wantErr bool
	}{
	...
	for _, tt := range tests {
		minecartMerge = tt.value
		// We can't call runMinecartCreate without bd, but we can test the
		...
		if minecartMerge != "" {
			...
			case "direct", "mr", "local":
				// Valid
			default:
				err = fmt.Errorf("invalid --merge value %q: must be direct, mr, or local", minecartMerge)
			}
		}
...
func TestMinecartCreate_NoArgsNoFlag(t *testing.T) {
	// Reset flags
	minecartFromEpic = ""
	minecartMerge = ""

	err := runMinecartCreate(minecartCreateCmd, []string{})
	if err == nil {
...
func TestMinecartCreate_FromEpicWithOverrideName(t *testing.T) {
	...
	if minecartCreateCmd.Args == nil {
		t.Fatal("minecartCreateCmd.Args should not be nil")
	}
	// cobra.ArbitraryArgs accepts any number of args including zero
	if err := minecartCreateCmd.Args(minecartCreateCmd, []string{}); err != nil {
		t.Errorf("ArbitraryArgs should accept zero args, got: %v", err)
	}
	if err := minecartCreateCmd.Args(minecartCreateCmd, []string{"Custom Name"}); err != nil {
...
func TestFromEpic_SlingableTypeFiltering(t *testing.T) {
	...
	for _, typ := range slingable {
		if !minecartops_IsSlingableType(typ) {
...
// minecartops_IsSlingableType wraps the real function to avoid import cycle issues
// in test. Uses the same logic as minecartops.IsSlingableType.
func minecartops_IsSlingableType(issueType string) bool {
	switch issueType {
