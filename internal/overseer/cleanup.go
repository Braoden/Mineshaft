...
const (
	acpPidFileName   = "overseer-acp.pid"
	acpAgentFileName = "overseer-acp.agent"
)
...
func ACPPidFilePath(townRoot string) string {
	return filepath.Join(townRoot, "overseer", acpPidFileName)
}
...
func ACPAgentFilePath(townRoot string) string {
	return filepath.Join(townRoot, "overseer", acpAgentFileName)
}
...
func (c *CleanupVetoChecker) ShouldVetoCleanup() (bool, string) {
	if IsACPActive(c.townRoot) {
		return true, "ACP session is active - Overseer may be reviewing worker diffs"
	}
	return false, ""
}
