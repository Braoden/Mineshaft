...
func GetPrefixForRig(townRoot, rigName string) string {
	...
	// Routes paths are like "mineshaft/overseer/rig" or "beads/overseer/rig"
	for _, r := range routes {
...
func CheckPrefixAvailable(townRoot string, prefix string, newPath string) error {
	...
	// since the same rig can have different path variants (e.g., "mineshaft" vs "mineshaft/overseer/rig").
...
// ResolveRepoAliasBeadsDir resolves a Mineshaft repo alias to its canonical
// .beads directory. Bare aliases are route names like "mineshaft" plus the
...
// bd keeps its native --repo behavior outside Mineshaft aliases.
