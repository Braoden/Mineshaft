...
func TestFeedNextReadyIssue_DispatchesFirstReadyIssue(t *testing.T) {
	...
	for _, iss := range []*beadsdk.Issue{minecart, closed, assigned, ready} {
		if err := store.CreateIssue(ctx, iss, "test"); err != nil {
			t.Fatalf("CreateIssue %s: %v", iss.ID, err)
		}
	}

	// Add tracks deps: minecart -> each tracked issue
...
func TestFeedNextReadyIssue_SkipsEpicAndDispatchesTask(t *testing.T) {
	...
	for _, iss := range []*beadsdk.Issue{minecart, epic, task} {
		if err := store.CreateIssue(ctx, iss, "test"); err != nil {
			t.Fatalf("CreateIssue %s: %v", iss.ID, err)
		}
	}

	// Add tracks deps: minecart -> epic, minecart -> task
...
func TestFeedNextReadyIssue_SkipsBlockedIssue(t *testing.T) {
	...
	minecart := &beadsdk.Issue{
		ID:        "test-minecart3",
		Title:     "Minecart For Blocked Test",
		Status:    beadsdk.StatusOpen,
		Priority:  2,
		IssueType: beadsdk.TypeTask,
		CreatedAt: now,
	...
	for _, iss := range []*beadsdk.Issue{minecart, blocker, blockedTask, unblockedTask} {
		if err := store.CreateIssue(ctx, iss, "test"); err != nil {
			t.Fatalf("CreateIssue %s: %v", iss.ID, err)
		}
	}

	// Add tracks deps from minecart
...
func TestFeedNextReadyIssue_NoReadyIssues_LogsMessage(t *testing.T) {
	...
	for _, iss := range []*beadsdk.Issue{minecart, closed1, closed2} {
		if err := store.CreateIssue(ctx, iss, "test"); err != nil {
			t.Fatalf("CreateIssue %s: %v", iss.ID, err)
		}
	}
...
func TestFeedNextReadyIssue_SkipsParkedRig(t *testing.T) {
	...
	for _, iss := range []*beadsdk.Issue{minecart, task} {
		if err := store.CreateIssue(ctx, iss, "test"); err != nil {
			t.Fatalf("CreateIssue %s: %v", iss.ID, err)
		}
	}
	...
	if !foundParked {
		// It's also possible we got "no ready issues" if getMinecartTrackedIssues
...
// ---------------------------------------------------------------------------
// DS-07: CheckMinecartsForIssue skips staged_ready minecarts
// ---------------------------------------------------------------------------

func TestCheckMinecartsForIssue_SkipsStagedReady(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()
	...
	// Create a minecart as open first (SDK validates status on create),
	// then transition to "staged_ready" via UpdateIssue.
	minecart := &beadsdk.Issue{
		ID:        "test-cv-staged1",
		Title:     "Staged Ready Minecart",
		Status:    beadsdk.StatusOpen,
		Priority:  2,
		IssueType: beadsdk.TypeTask,
		CreatedAt: now,
	...
	for _, iss := range []*beadsdk.Issue{minecart, tracked} {
		if err := store.CreateIssue(ctx, iss, "test"); err != nil {
			t.Fatalf("CreateIssue %s: %v", iss.ID, err)
		}
	}

	// Transition minecart to "staged_ready" (SDK validates status on create,
	// so we create as open first and update).
	if err := store.UpdateIssue(ctx, minecart.ID, map[string]interface{}{
		"status": "staged_ready",
	}, "test"); err != nil {
		t.Fatalf("UpdateIssue to staged_ready: %v", err)
	}

	// Add tracks dependency: minecart tracks the closed issue
	dep := &beadsdk.Dependency{
		IssueID:     minecart.ID,
	...
	// Call CheckMinecartsForIssue with the tracked issue's ID (simulating close event)
	result := CheckMinecartsForIssue(ctx, store, townRoot, tracked.ID, "DS-07", logger, gtPath, nil)

	// The minecart should be returned (it was found as a tracker)
	if len(result) == 0 {
		t.Skipf("no tracking minecarts found — GetDependentsWithMetadata may not work in embedded Dolt")
	}

	// Verify the staged minecart was skipped via log messages
	...
	if !foundStagedSkip {
		t.Errorf("expected log message about staged minecart being skipped, got: %v", *logMsgs)
	}

	// Verify "checking minecart" was NOT logged (minecart should be skipped before check)
	for _, msg := range *logMsgs {
		if strings.Contains(msg, "checking minecart") && strings.Contains(msg, minecart.ID) {
			t.Errorf("staged minecart should not have been checked, but found log: %s", msg)
		}
	}
}

// ---------------------------------------------------------------------------
// DS-08: CheckMinecartsForIssue skips staged_warnings minecarts
// ---------------------------------------------------------------------------

func TestCheckMinecartsForIssue_SkipsStagedWarnings(t *testing.T) {
	store, cleanup := setupTestStore(t)
	defer cleanup()
	...
	// Create a minecart as open first, then transition to "staged_warnings".
	minecart := &beadsdk.Issue{
		ID:        "test-cv-staged2",
		Title:     "Staged Warnings Minecart",
		Status:    beadsdk.StatusOpen,
		Priority:  2,
		IssueType: beadsdk.TypeTask,
		CreatedAt: now,
	...
	// Transition minecart to "staged_warnings"
	if err := store.UpdateIssue(ctx, minecart.ID, map[string]interface{}{
	...
	result := CheckMinecartsForIssue(ctx, store, townRoot, tracked.ID, "DS-08", logger, gtPath, nil)

	if len(result) == 0 {
		t.Skipf("no tracking minecarts found — GetDependentsWithMetadata may not work in embedded Dolt")
	}

	// Verify the staged minecart was skipped
	foundStagedSkip := false
	for _, msg := range *logMsgs {
		if strings.Contains(msg, "staged") && strings.Contains(msg, "skipping") {
			foundStagedSkip = true
	...
	// Verify "checking minecart" was NOT logged
	for _, msg := range *logMsgs {
		if strings.Contains(msg, "checking minecart") && strings.Contains(msg, minecart.ID) {
			t.Errorf("staged minecart should not have been checked, but found log: %s", msg)
		}
...
func TestCheckMinecartsForIssue_FeedsAfterStagedToOpenTransition(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on windows")
	}
	...
	// Create a minecart as open first, then transition to "staged_ready"
	minecart := &beadsdk.Issue{
		ID:        "test-cv-launch",
		Title:     "Launched Minecart",
		Status:    beadsdk.StatusOpen,
		Priority:  2,
		IssueType: beadsdk.TypeTask,
		CreatedAt: now,
	...
	// Transition minecart to "staged_ready"
	if err := store.UpdateIssue(ctx, minecart.ID, map[string]interface{}{
		"status": "staged_ready",
	}, "test"); err != nil {
		t.Fatalf("UpdateIssue to staged_ready: %v", err)
	...
	result1 := CheckMinecartsForIssue(ctx, store, townRoot, tracked.ID, "DS-10-staged", logger1, gtPath, nil)
	if len(result1) == 0 {
		t.Skipf("no tracking minecarts found — GetDependentsWithMetadata may not work in embedded Dolt")
	}
	...
	if !foundStagedSkip {
		t.Fatalf("minecart should have been skipped while staged, logs: %v", *logMsgs1)
	}

	// Phase 2: Transition minecart to "open" (launch it)
	if err := store.UpdateIssue(ctx, minecart.ID, map[string]interface{}{
	...
	if isMinecartStaged(ctx, store, minecart.ID) {
		t.Fatal("minecart should not be staged after transition to open")
	}

	// Phase 3: Call CheckMinecartsForIssue again — now the minecart should be processed
	logger2, logMsgs2 := makeLogger()
	_ = CheckMinecartsForIssue(ctx, store, townRoot, tracked.ID, "DS-10-open", logger2, gtPath, nil)

	// Verify "checking minecart" WAS logged (minecart is now open and being processed)
	foundChecking := false
	...
	if !foundChecking {
		t.Errorf("expected minecart to be checked after staged->open transition, logs: %v", *logMsgs2)
	}

	// Verify it was NOT skipped as staged
	for _, msg := range *logMsgs2 {
		if strings.Contains(msg, "staged") && strings.Contains(msg, "skipping") {
			t.Errorf("minecart should NOT be skipped after transition to open, but found: %s", msg)
		}
	}
}
...
func TestGetMinecartTrackedIssues_CrossRigFallback(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on windows")
	}
	...
	// Create minecart in the store (local)
	minecart := &beadsdk.Issue{
		ID:        "test-minecart-xrig",
		Title:     "Cross-Rig Minecart",
		Status:    beadsdk.StatusOpen,
		Priority:  2,
		IssueType: beadsdk.TypeTask,
		CreatedAt: now,
