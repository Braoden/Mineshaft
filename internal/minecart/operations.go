...
// CheckMinecartsForIssue finds any minecarts tracking the given issue and triggers
// minecart completion checks. If the minecart is not complete, it reactively feeds
// the next ready issue to keep the minecart progressing without waiting for
...
//   - store: beads storage for dependency/issue queries (nil skips minecart checks)
...
func CheckMinecartsForIssue(ctx context.Context, store beadsdk.Storage, townRoot, issueID, caller string, logger func(format string, args ...interface{}), gtPath string, isRigParked func(string) bool, resolver ...*StoreResolver) []string {
	if logger == nil {
	...
	// Find minecarts tracking this issue
	minecartIDs := getTrackingMinecarts(ctx, store, issueID, logger)
	...
	for _, minecartID := range minecartIDs {
		...
		if isMinecartStaged(ctx, store, minecartID) {
			logger("%s: minecart %s is staged (not yet launched), skipping", caller, minecartID)
			continue
		}

		logger("%s: checking minecart %s", caller, minecartID)
		if err := runMinecartCheck(ctx, townRoot, minecartID, gtPath); err != nil {
			logger("%s: minecart %s check failed: %s", caller, minecartID, util.FirstLine(err.Error()))
		}

		// Continuation feed: if minecart is still open after the completion check,
		// reactively dispatch the next ready issue. This makes minecart feeding
		// event-driven instead of relying on polling-based patrol cycles.
		if !isMinecartClosed(ctx, store, minecartID) {
			feedNextReadyIssue(ctx, store, townRoot, minecartID, caller, logger, gtPath, isRigParked, res)
		}
	}
...
// getTrackingMinecarts returns minecart IDs that track the given issue.
// Uses SDK GetDependentsWithMetadata filtered by type "tracks".
func getTrackingMinecarts(ctx context.Context, store beadsdk.Storage, issueID string, logger func(format string, args ...interface{})) []string {
	dependents, err := store.GetDependentsWithMetadata(ctx, issueID)
	if err != nil {
		if logger != nil {
			logger("Minecart: getTrackingMinecarts(%s) store error: %v", issueID, err)
		}
		return nil
	}

	minecartIDs := make([]string, 0)
	for _, d := range dependents {
		if string(d.DependencyType) == "tracks" {
			minecartIDs = append(minecartIDs, d.ID)
		}
	}
	return minecartIDs
}

// isMinecartClosed checks if a minecart is already closed.
func isMinecartClosed(ctx context.Context, store beadsdk.Storage, minecartID string) bool {
	issue, err := store.GetIssue(ctx, minecartID)
...
// isMinecartStaged checks if a minecart is in a staged state (not yet launched).
// Staged minecarts have statuses like "staged_ready" or "staged_warnings".
// They should not be fed until they are launched (transitioned to "open").
func isMinecartStaged(ctx context.Context, store beadsdk.Storage, minecartID string) bool {
	issue, err := store.GetIssue(ctx, minecartID)
	if err != nil || issue == nil {
...
// trackedIssue holds basic info about an issue tracked by a minecart.
...
func feedNextReadyIssue(ctx context.Context, store beadsdk.Storage, townRoot, minecartID, caller string, logger func(format string, args ...interface{}), gtPath string, isRigParked func(string) bool, resolver *StoreResolver) {
	...
	if minecart, err := store.GetIssue(ctx, minecartID); err == nil && minecart != nil {
	...
	for _, issue := range tracked {
		...
		// feature, chore) can be dispatched. Epics, minecarts, and other
		// container types are skipped.
		if !IsSlingableType(issue.IssueType) {
			logger("%s: minecart %s: %s has non-slingable type %q, skipping", caller, minecartID, issue.ID, issue.IssueType)
			continue
		}
		...
		if isIssueBlocked(ctx, store, issue.ID, resolver) {
			logger("%s: minecart %s: %s is blocked, skipping", caller, minecartID, issue.ID)
			continue
		}
		...
		if rig == "" {
			logger("%s: minecart %s: cannot determine rig for issue %s, skipping", caller, minecartID, issue.ID)
			continue
		}

		if isRigParked(rig) {
			logger("%s: minecart %s: rig %s is parked, skipping %s", caller, minecartID, rig, issue.ID)
			continue
		}

		logger("%s: minecart %s: feeding next ready issue %s to %s", caller, minecartID, issue.ID, rig)
		if err := dispatchIssue(ctx, townRoot, issue.ID, rig, gtPath, baseBranch); err != nil {
			logger("%s: minecart %s: dispatch %s failed: %s", caller, minecartID, issue.ID, util.FirstLine(err.Error()))
			continue // Try next issue on dispatch failure
		}
		return // Successfully dispatched one issue
	}

	logger("%s: minecart %s: no ready issues to feed", caller, minecartID)
}

// getMinecartTrackedIssues returns issues tracked by a minecart with fresh status.
...
func getMinecartTrackedIssues(ctx context.Context, store beadsdk.Storage, minecartID, townRoot string, resolver *StoreResolver) []trackedIssue {
	deps, err := store.GetDependenciesWithMetadata(ctx, minecartID)
	...
	// beads when minecarts live in hq), resolve via the StoreResolver which
