...
// the complete minecart feed workflow can be parsed and validated.
//
// This is a regression test for GitHub issue #1133:
// - mol-minecart-feed formula was committed with undefined template variables
...
// 3. The minecart input variable is marked as required
// 4. Computed variables have empty defaults (not required as inputs)
func TestMinecartFeedWorkflow_Integration(t *testing.T) {
	...
	// Step 3: Verify the minecart variable is required (this is the input)
	minecartVar, ok := f.Vars["minecart"]
	if !ok {
		t.Fatal("Missing required 'minecart' variable definition")
	}
	if !minecartVar.Required {
		t.Error("'minecart' variable should be marked as required")
	}
...
func TestAllDogFormulas_CanBeWisped(t *testing.T) {
	dogFormulas := []string{
		constants.MolMinecartFeed,
		constants.MolMinecartCleanup,
		"mol-dep-propagate",
		"mol-digest-generate",
		"mol-orphan-scan",
		"mol-session-gc",
...
// TestMinerFormulas_CanBeWisped verifies that miner formulas pass variable validation.
// These formulas are used when spawning miners for code review and PR review tasks.
func TestMinerFormulas_CanBeWisped(t *testing.T) {
	minerFormulas := []struct {
		name         string
		requiredVars []string
	}{
		{
			name:         "mol-miner-code-review",
			requiredVars: []string{"scope", "issue", "rig"},
		},
		{
			name:         "mol-miner-review-pr",
			requiredVars: []string{"pr_url", "issue", "rig"},
		},
	}

	formulasDir := "formulas"
	for _, tc := range minerFormulas {
