// Package miner provides miner workspace and session management.
package miner
...
// SessionManager handles miner session lifecycle.
type SessionManager struct {
	tmux *tmux.Tmux
	rig  *rig.Rig
}

// NewSessionManager creates a new miner session manager for a rig.
...
// SessionStartOptions configures miner session startup.
type SessionStartOptions struct {
	// WorkDir overrides the default working directory (miner clone dir).
	WorkDir string
...
// SessionInfo contains information about a running miner session.
type SessionInfo struct {
	// Miner is the miner name.
...
func (m *SessionManager) minerDir(miner string) string {
	return filepath.Join(m.rig.Path, "miners", miner)
}
...
func (m *SessionManager) clonePath(miner string) string {
	// New structure: miners/<name>/<rigname>/
	newPath := filepath.Join(m.rig.Path, "miners", miner, m.rig.Name)
	if info, err := os.Stat(newPath); err == nil && info.IsDir() {
		return newPath
	}

	// Old structure: miners/<name>/ (backward compat)
	oldPath := filepath.Join(m.rig.Path, "miners", miner)
...
// freshBranchName returns a unique branch name for a new miner session.
// Mirrors the naming convention in Manager.buildBranchName:
//   - miner/<name>/<issue>@<timestamp> when an issue is known
//   - miner/<name>-<timestamp> otherwise
//
// parseFreshBranchName is the structural inverse.
func (m *SessionManager) freshBranchName(minerName, issue string) string {
	ts := strconv.FormatInt(time.Now().UnixMilli(), 36)
	if issue != "" {
		return fmt.Sprintf("miner/%s/%s@%s", minerName, issue, ts)
	}
	return fmt.Sprintf("miner/%s-%s", minerName, ts)
}
...
func parseFreshBranchName(branch string) freshBranchMeta {
	...
	if slash := strings.Index(rest, "/"); slash >= 0 {
		...
		return freshBranchMeta{miner: name, issue: tail[:at], ok: true}
	}
	// miner/<name>-<ts> (no slash in rest)
	dash := strings.LastIndex(rest, "-")
	if dash <= 0 || dash == len(rest)-1 {
		return freshBranchMeta{}
	}
	return freshBranchMeta{miner: rest[:dash], ok: true}
}
...
// replace the worktree's current branch with a new miner branch based on
...
func shouldCreateFreshSessionBranch(currentBranch, issue, canonicalBranch string) bool {
	...
	// Same-issue respawn: keep the existing miner branch so preserved work
	// for this issue isn't discarded.
	if meta.ok && issue != "" && meta.issue == issue {
		return false
	}

	// On the canonical base branch — need a fresh miner branch to work on.
	if canonicalBranch != "" && currentBranch == canonicalBranch {
		return true
	}

	// On some other miner branch belonging to a different issue — fresh
	// branch is safer than inheriting unrelated preserved history.
	return issue != "" && meta.ok
}

func (m *SessionManager) ensureCanonicalSessionBranch(g *git.Git, miner string, opts SessionStartOptions) string {
	...
	// canonical remote base instead of any preserved local miner branch.
	...
	newBranch := m.freshBranchName(miner, opts.Issue)
...
// hasMiner checks if the miner exists in this rig.
func (m *SessionManager) hasMiner(miner string) bool {
	minerPath := m.minerDir(miner)
	info, err := os.Stat(minerPath)
	if err != nil {
		return false
	}
	return info.IsDir()
...
// minerSlot returns a unique integer slot index for this miner based on its
// position among existing miner directories. This enables port offsetting and
// resource isolation when multiple miners run in parallel (GH#954).
func (m *SessionManager) minerSlot(miner string) int {
...
// Start creates and starts a new session for a miner.
func (m *SessionManager) Start(miner string, opts SessionStartOptions) error {
	if !m.hasMiner(miner) {
		return fmt.Errorf("%w: %s", ErrMinerNotFound, miner)
	}

	sessionID := m.SessionName(miner)
	...
	// Ensure runtime settings exist in the shared miners parent directory.
	// Settings are passed to Claude Code via --settings flag.
	minerSettingsDir := config.RoleSettingsDir("miner", m.rig.Path)
	if err := runtime.EnsureSettingsForRole(minerSettingsDir, workDir, "miner", runtimeConfig); err != nil {
	...
	address := session.BeaconRecipient("miner", miner, m.rig.Name)
	...
	if g := git.NewGit(workDir); g != nil {
		minerGitBranch = m.ensureCanonicalSessionBranch(g, miner, opts)
	}
	// Generate the GASTA run ID — the root identifier for all telemetry emitted
	// by this miner session and its subprocesses (bd, mail, …).
	...
	if opts.Issue != "" {
		agentID := fmt.Sprintf("%s/miners/%s", m.rig.Name, miner)
	...
	// causing the miner to sit idle at an empty prompt. See GH#1379.
	if fallbackInfo.SendStartupNudge {
	...
	_ = runtime.RunStartupFallback(m.tmux, sessionID, "miner", runtimeConfig)
	...
	session.RecordAgentInstantiateFromDir(context.Background(), runID, runtimeConfig.ResolvedAgent,
		"miner", miner, sessionID, m.rig.Name, townRoot, opts.Issue, workDir)
...
// Stop terminates a miner session.
func (m *SessionManager) Stop(miner string, force bool) error {
	sessionID := m.SessionName(miner)
...
// IsRunning checks if a miner session is active and healthy.
...
func (m *SessionManager) IsRunning(miner string) (bool, error) {
	sessionID := m.SessionName(miner)
	status := m.tmux.CheckSessionHealth(sessionID, 0)
	return status == tmux.SessionHealthy, nil
}

// Status returns detailed status for a miner session.
func (m *SessionManager) Status(miner string) (*SessionInfo, error) {
	sessionID := m.SessionName(miner)
...
// This includes miners, witness, refinery, and crew sessions.
// Use ListMiners() to get only miner sessions.
func (m *SessionManager) List() ([]SessionInfo, error) {
	...
	for _, sessionID := range sessions {
		...
		miner := strings.TrimPrefix(sessionID, prefix)
...
// ListMiners returns information only about miner sessions for this rig.
// Filters out witness, refinery, and crew sessions.
func (m *SessionManager) ListMiners() ([]SessionInfo, error) {
	infos, err := m.List()
	if err != nil {
		return nil, err
	}
	...
	for _, info := range infos {
		// Skip non-miner sessions
		if info.Miner == "witness" || info.Miner == "refinery" || strings.HasPrefix(info.Miner, "crew-") {
			continue
		}
...
// Attach attaches to a miner session.
func (m *SessionManager) Attach(miner string) error {
	sessionID := m.SessionName(miner)
...
// Capture returns the recent output from a miner session.
func (m *SessionManager) Capture(miner string, lines int) (string, error) {
	sessionID := m.SessionName(miner)
...
// Inject sends a message to a miner session.
func (m *SessionManager) Inject(miner, message string) error {
	sessionID := m.SessionName(miner)
...
// StopAll terminates all miner sessions for this rig.
func (m *SessionManager) StopAll(force bool) error {
	infos, err := m.ListMiners()
	if err != nil {
		return err
	}
	...
	var errs []error
	for _, info := range infos {
		if err := m.Stop(info.Miner, force); err != nil {
			errs = append(errs, fmt.Errorf("stopping %s: %w", info.Miner, err))
		}
	}
...
// verifyStartupNudgeDelivery checks if the miner started working after the
...
// before Claude Code is ready, causing the miner to sit idle.
//
...
// The witness zombie patrol will eventually detect and handle truly idle miners.
...
// hookIssue pins an issue to a miner's hook using bd update.
