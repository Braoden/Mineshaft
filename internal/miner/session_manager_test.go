...
func TestHasMiner(t *testing.T) {
	root := t.TempDir()
	// hasMiner checks filesystem, so create actual directories
	for _, name := range []string{"Toast", "Cheedo"} {
		if err := os.MkdirAll(filepath.Join(root, "miners", name), 0755); err != nil {
	...
	if !m.hasMiner("Toast") {
		t.Error("expected hasMiner(Toast) = true")
	}
	if !m.hasMiner("Cheedo") {
		t.Error("expected hasMiner(Cheedo) = true")
	}
	if m.hasMiner("Unknown") {
		t.Error("expected hasMiner(Unknown) = false")
	}
}
...
func TestMinerCommandFormat(t *testing.T) {
	...
	for _, part := range requiredParts {
		if !strings.Contains(fullCommand, part) {
			t.Errorf("Miner command should contain %q", part)
		}
	}
...
func TestEnsureCanonicalSessionBranch_UsesOriginDefaultBranch(t *testing.T) {
	...
	if err := repoGit.CheckoutNewBranch("miner/toast-old", "main"); err != nil {
		t.Fatalf("checkout stale miner branch: %v", err)
	}
	...
	if err := repoGit.Commit("stale local miner commit"); err != nil {
...
`
	...
	minerWorkDir := filepath.Join(rigPath, "miners", "Toast")
	...
	tests := []struct {
		...
		},
		{
			...
			issueID:     "bd-xyz789",
			expectedDir: filepath.Join(townRoot, "beads/overseer/rig"),
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			resolved := m.resolveBeadsDir(tc.issueID, minerWorkDir)
			if resolved != tc.expectedDir {
				t.Errorf("resolveBeadsDir(%q, %q) = %q, want %q",
					tc.issueID, minerWorkDir, resolved, tc.expectedDir)
			}
		})
	}
}
...
func TestPromptlessFallbackIncludesPrimeAndWorkInstructions(t *testing.T) {
	beaconConfig := session.BeaconConfig{
		Recipient:               session.BeaconRecipient("miner", "toast", "demo"),
		Sender:                  "witness",
		Topic:                   "assigned",
		MolID:                   "demo-123",
...
// miner startup because the function sleeps before its first idle check.
...
func TestMinerSlot(t *testing.T) {
	tmpDir := t.TempDir()
	rigPath := tmpDir
	...
	// No miners — should return 0
	if slot := sm.minerSlot("alpha"); slot != 0 {
		t.Errorf("empty dir: got slot %d, want 0", slot)
	}

	// Create some miner dirs (sorted: alpha, beta, gamma)
	for _, name := range []string{"alpha", "beta", "gamma"} {
		if err := os.MkdirAll(filepath.Join(minersDir, name), 0755); err != nil {
			t.Fatal(err)
		}
	}
	...
	for _, tt := range tests {
		if slot := sm.minerSlot(tt.name); slot != tt.want {
			t.Errorf("minerSlot(%q) = %d, want %d", tt.name, slot, tt.want)
		}
	}

	// Hidden dirs should be skipped
	if err := os.MkdirAll(filepath.Join(minersDir, ".hidden"), 0755); err != nil {
		t.Fatal(err)
	}
	if slot := sm.minerSlot("beta"); slot != 1 {
		t.Errorf("with hidden dir: minerSlot(beta) = %d, want 1", slot)
	}
}
...
func TestParseFreshBranchName_RoundTrip(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			branch := sm.freshBranchName(c.miner, c.issue)
			meta := parseFreshBranchName(branch)
			if !meta.ok {
				t.Fatalf("parseFreshBranchName(%q) not ok", branch)
			}
			if meta.miner != c.miner {
				t.Errorf("miner = %q, want %q (from %q)", meta.miner, c.miner, branch)
			}
