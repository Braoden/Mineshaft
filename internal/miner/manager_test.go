...
// This allows miner tests to run without a real bd installation.
//
...
func installMockBd(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		psScript := `# Mock bd for miner tests (PowerShell)
$cmd = ''
foreach ($arg in $args) {
...
func setupCanonicalBranchManagerTest(t *testing.T) (*Manager, string) {
	...
	root := t.TempDir()
	overseerRig := filepath.Join(root, "overseer", "rig")
	if err := os.MkdirAll(overseerRig, 0755); err != nil {
	...
	overseerBeads := filepath.Join(overseerRig, ".beads")
	...
	readmePath := filepath.Join(overseerRig, "README.md")
	if err := os.WriteFile(readmePath, []byte("# Test\n"), 0644); err != nil {
		t.Fatalf("write README.md: %v", err)
	}
	overseerGit := git.NewGit(overseerRig)
	if err := overseerGit.Add("README.md"); err != nil {
		t.Fatalf("git add: %v", err)
	}
	if err := overseerGit.Commit("Initial commit"); err != nil {
		t.Fatalf("git commit: %v", err)
	}

	cmd = exec.Command("git", "remote", "add", "origin", overseerRig)
	cmd.Dir = overseerRig
	if out, err := cmd.CombinedOutput(); err != nil {
	...
	return NewManager(r, git.NewGit(root), nil), overseerRig
}

func createStaleMinerCommit(t *testing.T, repoPath, startPoint, branchName string) string {
	t.Helper()
	...
	if err := repoGit.Commit("Create stale miner branch"); err != nil {
...
func TestListEmpty(t *testing.T) {
	...
	if len(miners) != 0 {
		t.Errorf("miners count = %d, want 0", len(miners))
	}
}
...
func TestGetNotFound(t *testing.T) {
	...
	if err != ErrMinerNotFound {
		t.Errorf("Get = %v, want ErrMinerNotFound", err)
	}
}
...
func TestRemoveNotFound(t *testing.T) {
	...
	if err != ErrMinerNotFound {
		t.Errorf("Remove = %v, want ErrMinerNotFound", err)
	}
}
...
func TestMinerDir(t *testing.T) {
	...
	expected := "/home/user/ai/test-rig/miners/Toast"
...
func TestAssigneeID(t *testing.T) {
	...
	expected := "test-rig/miners/Toast"
	if id != expected {
...
func TestNewManager_NamepoolFromRigConfig(t *testing.T) {
	...
	// Write rig config.json with miner_names (no settings/config.json)
	rigConfig := `{"miner_names": ["alpha", "beta", "gamma"]}`
...
func TestGetReturnsWorkingWithoutBeads(t *testing.T) {
	...
	// (assume the miner is doing something if it exists)
	//
	...
	root := t.TempDir()
	minerDir := filepath.Join(root, "miners", "Test")
	if err := os.MkdirAll(minerDir, 0755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}

	// Create overseer/rig directory for beads (but no actual beads)
	overseerRigDir := filepath.Join(root, "overseer", "rig")
	...
	// Get should return miner with StateWorking (assume active if beads unavailable)
	miner, err := m.Get("Test")
	if err != nil {
		t.Fatalf("Get: %v", err)
	}

	if miner.Name != "Test" {
		t.Errorf("Name = %q, want Test", miner.Name)
	}
	if miner.State != StateWorking {
		t.Errorf("State = %v, want StateWorking (beads not available)", miner.State)
	}
}

func TestListWithMiners(t *testing.T) {
	root := t.TempDir()

	// Create some miner directories (state is now derived from beads, not state files)
	for _, name := range []string{"Toast", "Cheedo"} {
		minerDir := filepath.Join(root, "miners", name)
		if err := os.MkdirAll(minerDir, 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
	}
	if err := os.MkdirAll(filepath.Join(root, "miners", ".claude"), 0755); err != nil {
		t.Fatalf("mkdir .claude: %v", err)
	}
	// Create overseer/rig for beads path
	overseerRig := filepath.Join(root, "overseer", "rig")
	if err := os.MkdirAll(overseerRig, 0755); err != nil {
		t.Fatalf("mkdir overseer/rig: %v", err)
	}
	...
	if len(miners) != 2 {
		t.Errorf("miners count = %d, want 2", len(miners))
	}
}
...
// miner lifecycle operations that don't require beads.
...
func TestAddWithOptions_HasAgentsMD(t *testing.T) {
	// This test verifies that AGENTS.md exists in miner worktrees after creation.
	// AGENTS.md is critical for miners to "land the plane" properly.
	installMockBd(t)

	root := t.TempDir()

	// Create overseer/rig directory structure (this acts as repo base when no .repo.git)
	overseerRig := filepath.Join(root, "overseer", "rig")
	if err := os.MkdirAll(overseerRig, 0755); err != nil {
		t.Fatalf("mkdir overseer/rig: %v", err)
	}

	// Initialize git repo in overseer/rig
	...
	agentsMDContent := []byte("# AGENTS.md\n\nTest content for miners.\n")
	agentsMDPath := filepath.Join(overseerRig, "AGENTS.md")
	...
	overseerGit := git.NewGit(overseerRig)
	if err := overseerGit.Add("AGENTS.md"); err != nil {
		t.Fatalf("git add: %v", err)
	}
	if err := overseerGit.Commit("Add AGENTS.md"); err != nil {
		t.Fatalf("git commit: %v", err)
	}
	...
	// Create rig-level .beads directory with redirect to overseer/rig/.beads
	rigBeads := filepath.Join(root, ".beads")
	if err := os.MkdirAll(rigBeads, 0755); err != nil {
		t.Fatalf("mkdir rig .beads: %v", err)
	}
	...
	if err := os.WriteFile(rigRedirect, []byte("overseer/rig/.beads\n"), 0644); err != nil {
		t.Fatalf("write rig redirect: %v", err)
	}
	...
	// Create miner via AddWithOptions
	miner, err := m.AddWithOptions("TestAgent", AddOptions{})
	if err != nil {
		t.Fatalf("AddWithOptions: %v", err)
	}

	// Verify AGENTS.md exists in the worktree
	worktreeAgentsMD := filepath.Join(miner.ClonePath, "AGENTS.md")
...
//	true   | false      | in-use (normal finished miner)
//	false  | true       | orphan → kill session, available
//	true   | true       | in-use (normal working miner)
...
func TestReconcilePoolWith_KeepsDirBackedStaleSession(t *testing.T) {
	...
	activeName := "toast"
	orphanName := "nux"
	activeSession := session.MinerSessionName(session.PrefixFor("myrig"), activeName)
	orphanSession := session.MinerSessionName(session.PrefixFor("myrig"), orphanName)
...
func TestBuildBranchName(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			template: "",
			issue:    "",
			want:     "miner/alpha-", // timestamp suffix varies
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.template != "" {
				origDefault := rig.SystemDefaults["miner_branch_template"]
				rig.SystemDefaults["miner_branch_template"] = tt.template
				defer func() {
					rig.SystemDefaults["miner_branch_template"] = origDefault
				}()
			}
...
func TestAddWithOptions_NoPrimeMDCreatedLocally(t *testing.T) {
	...
	// Create overseer/rig directory structure
	overseerRig := filepath.Join(root, "overseer", "rig")
	if err := os.MkdirAll(overseerRig, 0755); err != nil {
		t.Fatalf("mkdir overseer/rig: %v", err)
	}
	...
	// Create redirect at rig level pointing to overseer/rig/.beads
	overseerBeads := filepath.Join(overseerRig, ".beads")
	if err := os.MkdirAll(overseerBeads, 0755); err != nil {
		t.Fatalf("mkdir overseer/rig/.beads: %v", err)
	}
	...
	dummyPath := filepath.Join(overseerRig, "README.md")
	...
	if err := overseerGit.Commit("Initial commit without PRIME.md"); err != nil {
		t.Fatalf("git commit: %v", err)
	}
	...
	// Create miner
	miner, err := m.AddWithOptions("TestNoLocal", AddOptions{})
	if err != nil {
		t.Fatalf("AddWithOptions: %v", err)
	}

	// BUG CHECK: The worktree should NOT have a local .beads/PRIME.md
	// ProvisionPrimeMDForWorktree should follow redirect to overseer/rig/.beads
	worktreePrimeMD := filepath.Join(miner.ClonePath, ".beads", "PRIME.md")
	...
	worktreeRedirect := filepath.Join(miner.ClonePath, ".beads", "redirect")
	if _, err := os.Stat(worktreeRedirect); os.IsNotExist(err) {
		t.Errorf("redirect file should exist at: %s", worktreeRedirect)
	}

	// Verify PRIME.md was created at overseer/rig/.beads/ (where redirect points)
	overseerPrimeMD := filepath.Join(overseerBeads, "PRIME.md")
	if _, err := os.Stat(overseerPrimeMD); os.IsNotExist(err) {
		t.Errorf("PRIME.md should exist at overseer/rig/.beads/: %s", overseerPrimeMD)
	}
}
...
func TestAddWithOptions_UsesCanonicalOriginDefaultBranch(t *testing.T) {
	...
	overseerGit := git.NewGit(overseerRig)
	baseSHA, err := overseerGit.Rev("origin/main")
	if err != nil {
		t.Fatalf("resolve origin/main: %v", err)
	}
	staleSHA := createStaleMinerCommit(t, overseerRig, "main", "miner/stale-source")

	miner, err := mgr.AddWithOptions("toast", AddOptions{})
	if err != nil {
		t.Fatalf("AddWithOptions: %v", err)
	}

	worktreeGit := git.NewGit(miner.ClonePath)
	staleAncestor, err := worktreeGit.IsAncestor(staleSHA, miner.Branch)
	if err != nil {
		t.Fatalf("check stale ancestry: %v", err)
	}
	if staleAncestor {
		t.Fatalf("new miner branch %q unexpectedly includes stale local commit %s", miner.Branch, staleSHA)
	}

	baseAncestor, err := worktreeGit.IsAncestor(baseSHA, miner.Branch)
	if err != nil {
		t.Fatalf("check canonical ancestry: %v", err)
	}
	if !baseAncestor {
		t.Fatalf("new miner branch %q should descend from origin/main commit %s", miner.Branch, baseSHA)
	}
}
...
func TestAllocateAndAdd_RunsWispSetupCommand(t *testing.T) {
	...
	_, miner, err := mgr.AllocateAndAdd(AddOptions{})
	if err != nil {
		t.Fatalf("AllocateAndAdd: %v", err)
	}

	data, err := os.ReadFile(filepath.Join(miner.ClonePath, "setup-marker"))
	if err != nil {
...
func TestAddWithOptions_SetupCommandFailureRollsBack(t *testing.T) {
	...
	minerDir := filepath.Join(mgr.rig.Path, "miners", "toast")
	if _, statErr := os.Stat(minerDir); !os.IsNotExist(statErr) {
		t.Fatalf("miner dir %s still exists after setup_command rollback", minerDir)
	}
}

func TestReuseIdleMiner_RunsSetupCommand(t *testing.T) {
	mgr, _ := setupCanonicalBranchManagerTest(t)
	...
	if reused.ClonePath != miner.ClonePath {
		t.Fatalf("reused clone path = %q, want %q", reused.ClonePath, miner.ClonePath)
	}
...
func TestReuseIdleMiner_SetupCommandFailureCleansWorktree(t *testing.T) {
	mgr, _ := setupCanonicalBranchManagerTest(t)
...
func TestReuseIdleMiner_UsesCanonicalOriginDefaultBranch(t *testing.T) {
	mgr, overseerRig := setupCanonicalBranchManagerTest(t)
...
func TestAddWithOptions_ResumeBranch(t *testing.T) {
	...
	// open PR that a miner needs to resume.
	prBranch := "miner/example/gh-1234@abcdef"
	prCommit := createStaleMinerCommit(t, overseerRig, "main", prBranch)
	...
	miner, err := mgr.AddWithOptions("toast", AddOptions{ResumeBranch: prBranch})
	if err != nil {
		t.Fatalf("AddWithOptions with ResumeBranch: %v", err)
	}

	if miner.Branch != prBranch {
		t.Fatalf("miner.Branch = %q, want %q (ResumeBranch should override fresh-branch naming)", miner.Branch, prBranch)
	}
...
func TestAddWithOptions_NoFilesAddedToRepo(t *testing.T) {
	// This test verifies the invariant that miner creation does NOT add any
	// TRACKED files to the repo's directory structure. The user's code should stay pure.
	//
	// After miner install, `git status` in the worktree should show no
	// untracked files and no modifications. Settings are installed at the shared
	// miners/.claude/settings.json directory (outside worktrees), so they
	...
	srcDir := filepath.Join(overseerRig, "src")
	...
	// Commit everything
	overseerGit := git.NewGit(overseerRig)
	if err := overseerGit.Add("."); err != nil {
		t.Fatalf("git add: %v", err)
	}
	if err := overseerGit.Commit("Initial commit - clean repo"); err != nil {
		t.Fatalf("git commit: %v", err)
	}
	...
	// Create AGENTS.md in overseer/rig AFTER git commit (NOT tracked in git)
	// This triggers the fallback copy during miner install
	agentsMDPath := filepath.Join(overseerRig, "AGENTS.md")
	if err := os.WriteFile(agentsMDPath, []byte("# AGENTS\n\nFallback content.\n"), 0644); err != nil {
		t.Fatalf("write AGENTS.md: %v", err)
	}

	// Create rig and miner manager
	r := &rig.Rig{
		Name: "rig",
		Path: root,
	}
	...
	// Create miner
	miner, err := m.AddWithOptions("TestClean", AddOptions{})
	if err != nil {
		t.Fatalf("AddWithOptions: %v", err)
	}

	// Run git status in worktree - should show nothing except .beads/ (infrastructure)
	// Settings are at miners/.claude/settings.json (outside worktree) so won't appear
	cmd = exec.Command("git", "status", "--porcelain")
	cmd.Dir = miner.ClonePath
	...
	if len(unexpected) > 0 {
		t.Errorf("miner worktree should be clean after install (no files added to repo), but git status shows:\n%s", strings.Join(unexpected, "\n"))
	}
}

func TestAddWithOptions_SettingsInstalledInMinersDir(t *testing.T) {
	// This test verifies that miner creation installs .claude/settings.json
	// in the SHARED miners/ parent directory (not inside individual worktrees).
	// Claude Code with --settings supports parent directory settings, and placing
	// them at the miners/ level avoids polluting individual worktree repos.
	...
	// Create miner
	miner, err := m.AddWithOptions("TestSettings", AddOptions{})
	if err != nil {
		t.Fatalf("AddWithOptions: %v", err)
	}

	// Verify settings.json exists in the SHARED miners/ parent directory
	// miners dir is the parent of miner.ClonePath's parent (ClonePath = miners/<name>/<rig>)
	minersDir := filepath.Dir(filepath.Dir(miner.ClonePath))
	settingsPath := filepath.Join(minersDir, ".claude", "settings.json")
	if _, err := os.Stat(settingsPath); os.IsNotExist(err) {
		t.Errorf("settings.json should exist at %s (shared miners dir) for Claude Code to find hooks", settingsPath)
	}

	// Verify settings.json does NOT exist inside the worktree (no longer installed there)
	worktreeSettingsPath := filepath.Join(miner.ClonePath, ".claude", "settings.json")
	if _, err := os.Stat(worktreeSettingsPath); err == nil {
		t.Errorf("settings.json should NOT exist inside worktree at %s (settings are now in shared miners dir)", worktreeSettingsPath)
	}
}
...
func TestPendingMarkerBlocksReallocation(t *testing.T) {
	...
	// Simulate AllocateName: create miners/ dir and write a .pending marker
	// for "furiosa" (as if AllocateName ran but AddWithOptions hasn't yet).
	minersDir := filepath.Join(tmpDir, "miners")
...
// TestStalePendingMarkerIsCleanedUp verifies that cleanupOrphanMinerState
...
func TestStalePendingMarkerIsCleanedUp(t *testing.T) {
	...
	// cleanupOrphanMinerState should remove stale markers.
	m.cleanupOrphanMinerState()

	if _, err := os.Stat(pendingPath); !os.IsNotExist(err) {
		t.Errorf("stale .pending file was not cleaned up by cleanupOrphanMinerState")
	}
}
...
func TestAddWithOptions_RollbackReleasesName(t *testing.T) {
	...
	// Create overseer/rig directory structure (acts as repo base)
	overseerRig := filepath.Join(root, "overseer", "rig")
	if err := os.MkdirAll(overseerRig, 0755); err != nil {
		t.Fatalf("mkdir overseer/rig: %v", err)
	}
	...
	if err := os.WriteFile(filepath.Join(overseerRig, "README.md"), []byte("# Test\n"), 0644); err != nil {
	...
	// Try to create miner — should fail because origin/main doesn't exist
	...
	// Verify miner directory was cleaned up
	minerDir := m.minerDir(name)
	if _, err := os.Stat(minerDir); !os.IsNotExist(err) {
		t.Errorf("miner directory %s still exists after failed AddWithOptions", minerDir)
	}
...
`
	...
	minerDir := m.minerDir(name)
	if _, err := os.Stat(minerDir); !os.IsNotExist(err) {
		t.Errorf("miner directory %s still exists after rollback", minerDir)
	}
	...
	clonePath := filepath.Join(minerDir, r.Name)
...
func TestManagerAgentLifecycleUsesTownBeadsDir(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(rigPath, ".beads", "redirect"), []byte("overseer/rig/.beads\n"), 0644); err != nil {
		t.Fatalf("write redirect: %v", err)
	}
...
`, logPath, townBeadsDir)
	...
	if err := m.createAgentBeadWithRetry(agentID, &beads.AgentFields{RoleType: "miner", Rig: rigName, AgentState: "spawning"}); err != nil {
...
// calls never produce duplicate miner names (GH#2215). Each goroutine will
...
// TestReuseIdleMiner_KillsLiveSession verifies that ReuseIdleMiner kills
...
func TestReuseIdleMiner_KillsLiveSession(t *testing.T) {
	...
	// Create minimal miner directory structure
	minerDir := filepath.Join(rigPath, "miners", minerName)
	if err := os.MkdirAll(minerDir, 0755); err != nil {
		t.Fatalf("mkdir miner dir: %v", err)
	}
	...
	// Call ReuseIdleMiner — it will kill the session, then fail on worktree
	// operations (no real git repo). The important thing is it does NOT return
	// ErrSessionRunning.
	_, reuseErr := mgr.ReuseIdleMiner(minerName, AddOptions{})

	// Verify it did NOT return ErrSessionRunning (the old buggy behavior)
	if errors.Is(reuseErr, ErrSessionRunning) {
		t.Fatalf("ReuseIdleMiner returned ErrSessionRunning for live session — " +
			"this is the sling-reuse-stale-session bug: idle miners with live " +
	...
	if !strings.Contains(reuseErr.Error(), "worktree") {
		t.Logf("ReuseIdleMiner error (expected worktree-related): %v", reuseErr)
	}

	// Verify the session was killed
	running, _ = tm.HasSession(sessionName)
	if running {
		t.Error("session should have been killed by ReuseIdleMiner")
	}
...
func TestRepairWorktreeWithOptions_KillsLiveSession(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(overseerRig, ".beads"), 0755); err != nil {
		t.Fatalf("mkdir overseer beads: %v", err)
	}
	if err := os.WriteFile(filepath.Join(rigPath, ".beads", "redirect"), []byte("overseer/rig/.beads\n"), 0644); err != nil {
	...
	minerName := "toast"
	oldClonePath := filepath.Join(rigPath, "miners", minerName, rigName)
	if err := overseerGit.WorktreeAddFromRef(oldClonePath, "old-toast", "HEAD"); err != nil {
...
// TestReuseIdleMiner_KillsStaleSession verifies that ReuseIdleMiner also
...
func TestReuseIdleMiner_KillsStaleSession(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("tmux not supported on Windows")
	}
	if _, err := exec.LookPath("tmux"); err != nil {
	...
	if errors.Is(reuseErr, ErrSessionRunning) {
		t.Fatal("ReuseIdleMiner should not return ErrSessionRunning for stale session")
	}
...
func TestReuseIdleMiner_NoSessionNoop(t *testing.T) {
	...
	// Should not return ErrSessionRunning
	if errors.Is(reuseErr, ErrSessionRunning) {
		t.Fatal("ReuseIdleMiner should not return ErrSessionRunning when no session exists")
	}
