...
// classify a miner consistently across list, recovery, witness, and capacity.
...
// WorkstateDisposition is the canonical miner lifecycle decision. It is pure
// policy: callers gather facts, this classifier decides how every subsystem
// should present and count the miner.
...
// DecideWorkstate returns the canonical disposition for a miner.
func DecideWorkstate(in WorkstateInput) WorkstateDisposition {
	...
	d.SafeToNuke = true
	d.Reason = "reusable"
	if strings.HasPrefix(in.Branch, "miner/") {
