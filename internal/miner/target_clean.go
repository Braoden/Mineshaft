// Package miner — target_clean.go
//
// Per-bead `target/` clean hook for reused miners (hq-x0v7v).
//
// Rust miners accumulate large `target/` directories (30-50 GB each) when a
...
// miner for a new bead, optionally delete the miner's `target/` directory
// before the new work attaches. The cleaner is policy-driven so operators can
// pick between always, every-N-beads, or never.
//
// Policy is parsed from a single config string at `miner.target_clean_policy`:
//   - "per_bead"           (default) clean on every reuse
//   - "every_n_beads:<N>"  clean once every N reuses (N >= 1)
//   - "never"              skip the hook entirely
//
// The counter for `every_n_beads` is persisted alongside the miner at
// `<minerDir>/target_clean_counter`. Cleaner state is intentionally
...
func (m *Manager) targetCleanPolicy() TargetCleanPolicy {
	...
	if err != nil || settings == nil || settings.Miner == nil {
		return DefaultTargetCleanPolicy()
	}
	policy, err := ParseTargetCleanPolicy(settings.Miner.TargetCleanPolicy)
	if err != nil {
		return DefaultTargetCleanPolicy()
	}
	return policy
...
// targetCleanCounterFile is the per-miner counter path. Keeps every_n_beads
// state out of the agent-bead schema and reset-safe across daemon restarts.
func targetCleanCounterFile(minerDir string) string {
	return filepath.Join(minerDir, "target_clean_counter")
}

// readTargetCleanCounter returns the persisted counter for this miner.
// Missing/garbage files return 0 (not an error — first reuse, fresh miner).
func readTargetCleanCounter(minerDir string) int {
	data, err := os.ReadFile(targetCleanCounterFile(minerDir))
	if err != nil {
		return 0
	}
...
func writeTargetCleanCounter(minerDir string, n int) error {
	return os.WriteFile(targetCleanCounterFile(minerDir), []byte(strconv.Itoa(n)), 0o644)
}
...
//     lives under the miner root, which this function cannot do alone).
...
// RunTargetCleanHook is the miner-reuse cleaner entry point.
//
// Given the miner's directory (for counter state) and worktree clone path
...
func RunTargetCleanHook(minerDir, clonePath string, policy TargetCleanPolicy) (string, error) {
	if policy.Mode == TargetCleanModeNever {
		return "", nil
	}

	counter := readTargetCleanCounter(minerDir) + 1
	// Persist the bumped counter unconditionally so every_n_beads stays accurate.
	if err := writeTargetCleanCounter(minerDir, counter); err != nil {
