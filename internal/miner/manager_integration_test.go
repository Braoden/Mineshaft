...
var minerManagerIntegrationCounter atomic.Int32
...
func TestManagerGetPrefersHookedBeadOverStaleAgentHook(t *testing.T) {
	...
	n := minerManagerIntegrationCounter.Add(1)
	...
	if err := os.MkdirAll(overseerRigPath, 0755); err != nil {
		t.Fatalf("mkdir overseer rig path: %v", err)
	}
	if err := os.MkdirAll(filepath.Join(rigPath, "miners", "toast"), 0755); err != nil {
		t.Fatalf("mkdir miner dir: %v", err)
	}

	// Rig .beads redirects to overseer/rig/.beads so NewManager resolves correctly.
	...
	routes := []beads.Route{
		{Prefix: "hq-", Path: "."},
		{Prefix: prefix + "-", Path: filepath.Join(rigName, "overseer", "rig")},
	}
	if err := beads.WriteRoutes(townBeadsDir, routes); err != nil {
		t.Fatalf("write routes: %v", err)
	}

	initBeadsDBWithPrefix(t, overseerRigPath, prefix)
	...
	if p.State != StateWorking {
		t.Fatalf("miner state = %q, want %q", p.State, StateWorking)
	}
	if p.Issue != current.ID {
		t.Fatalf("miner issue = %q, want hooked issue %q (stale hook %q)", p.Issue, current.ID, stale.ID)
	}
}
...
func TestManagerTreatsLiveSessionWithoutWorkAsReviewNeeded(t *testing.T) {
	...
	if p.State != StateReviewNeeded {
		t.Fatalf("miner state = %q, want %q when tmux session is alive without work", p.State, StateReviewNeeded)
	}
	if p.Issue != "" {
		t.Fatalf("miner issue = %q, want empty when no active hooked/assigned work exists", p.Issue)
	}

	idle, err := mgr.FindIdleMiner()
	if err != nil {
		t.Fatalf("mgr.FindIdleMiner(): %v", err)
	}
	if idle != nil {
		t.Fatalf("FindIdleMiner() = %q, want nil while session %s needs review", idle.Name, sessionName)
	}
}
