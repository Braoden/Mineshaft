...
// These names must never be allocated to miners.
...
// Built-in themes with themed miner names.
var BuiltinThemes = map[string][]string{
	...
	"wasteland": {
		...
		"boss", "sentinel", "paladin", "scribe", "initiate",
...
type NamePool struct {
	...
	// Never persist - always discover from existing miner directories.
...
// Called when a miner is nuked - the name becomes available for new miners.
// NOTE: This releases the NAME, not the miner. The miner is gone (nuked).
...
// MarkInUse marks a name as in use (for reconciling with existing miners).
...
// Reconcile updates the pool state based on existing miner directories.
// This should be called on startup to sync pool state with reality.
func (p *NamePool) Reconcile(existingMiners []string) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	// Mark all existing miners as in use
	for _, name := range existingMiners {
		if p.isThemedName(name) {
			p.InUse[name] = true
			continue
		}
...
// This ensures miner names are unique across rigs by giving each rig a different theme.
...
// ValidatePoolName validates a miner name for use in a theme.
...
func FindRigsUsingTheme(townRoot, theme string) []string {
	rigsFile := filepath.Join(townRoot, "overseer", "rigs.json")
