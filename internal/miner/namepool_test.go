...
func TestNamePool_Reconcile(t *testing.T) {
	...
	// Simulate existing miners from filesystem
...
func TestNamePool_ReconcileAdvancesOverflowPastNumericMiners(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "namepool-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNamePool_ReservedNamesInCustomNames(t *testing.T) {
	...
	custom := []string{"alpha", "witness", "beta", "overseer", "gamma"}
	...
	if allocated["overseer"] {
		t.Error("allocated reserved name 'overseer' from custom names")
	}
...
func TestSaveCustomTheme_AppendName(t *testing.T) {
	...
	// Read back, append, save again (simulates what `ms namepool add` does)
...
func TestFindRigsUsingTheme(t *testing.T) {
	...
	// Create overseer/rigs.json with two rigs
