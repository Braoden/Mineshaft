...
func TestBuildCommand_Claude(t *testing.T) {
	...
	if !strings.Contains(content, "allowed-tools: Bash(ms handoff:*)") {
	...
	if !strings.Contains(content, "ms handoff -y") {
		t.Error("Claude handoff command should use ms handoff -y")
	}
}
