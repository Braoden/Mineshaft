...
// writeFakeTmuxWithSession creates a fake tmux binary that reports the Supervisor
// session as existing (has-session returns 0). Used for supervisor idle guard tests
// where the session must be present so checkSupervisorHeartbeat reaches the nudge path.
...
// TestCheckSupervisorHeartbeat_IdleGuard verifies that the nudge is suppressed when
// the Supervisor heartbeat is stale but no active work is in flight (idle guard).
func TestCheckSupervisorHeartbeat_IdleGuard(t *testing.T) {
