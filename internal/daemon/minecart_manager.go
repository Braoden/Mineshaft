...
const (
	...
	// minecartGracePeriod is how long after creation a minecart is immune from
...
// MinecartManager monitors beads events for issue closes and periodically scans for stranded minecarts.
// It handles both event-driven completion checks (via minecart.CheckMinecartsForIssue) and periodic
// stranded minecart feeding/cleanup.
//
// Event polling watches ALL beads stores (town-level hq + per-rig) so that close events from
// any rig are detected. Minecarts live in the hq store, so minecart lookups always use hqStore.
...
type MinecartManager struct {
	...
	// duplicate minecart checks for the same stranded minecart.
	scanMu sync.Mutex
	...
	// been processed. This prevents duplicate minecart checks when the same close
...
// NewMinecartManager creates a new minecart manager.
...
// nil stores disables event-driven minecart checks (stranded scan still runs),
...
// Start begins the minecart manager goroutines (event poll + stranded scan).
// It is safe to call multiple times; subsequent calls are no-ops.
func (m *MinecartManager) Start() error {
	if !m.started.CompareAndSwap(false, true) {
		m.logger("Minecart: Start() already called, ignoring duplicate")
		return nil
	}
	m.wg.Add(2)
	go m.runEventPoll()
	go m.runStrandedScan()
	// Run a one-shot sweep to catch minecarts that completed during any previous
	// outage or while the daemon was stopped.
	go m.runStartupSweep()
	return nil
}

// Stop gracefully stops the minecart manager and closes any beads stores it owns.
func (m *MinecartManager) Stop() {
	m.cancel()
	m.wg.Wait()
	...
	for name, store := range stores {
		if store != nil {
			if err := store.Close(); err != nil {
				m.logger("Minecart: error closing beads store (%s): %v", name, err)
			} else {
				m.logger("Minecart: closed beads store (%s)", name)
			}
		}
	}
}
...
func (m *MinecartManager) runEventPoll() {
	defer m.wg.Done()
	...
	if !hasStores && !hasOpener {
		m.logger("Minecart: no beads stores and no opener, event polling disabled")
		return
	}
		...
		case <-ticker.C:
			...
			if hadError {
				...
				if newInterval != currentInterval {
					...
					m.logger("Minecart: poll backoff → %s", currentInterval)
				}
			} else if currentInterval != eventPollInterval {
				currentInterval = eventPollInterval
				ticker.Reset(currentInterval)
				m.logger("Minecart: poll recovered, interval reset to %s", currentInterval)
			}
		}
	}
}
...
func (m *MinecartManager) pollStoresSnapshot(stores map[string]beadsdk.Storage) bool {
...
// Minecart lookups always use the hq store since minecarts are hq-* prefixed.
...
func (m *MinecartManager) pollStore(name string, store beadsdk.Storage, stores map[string]beadsdk.Storage, seen map[string]bool) error {
	...
	if err != nil {
		if isInfNaNError(err) {
			...
			// minecart scanner will catch any completions that were lost.
			now := time.Now().UTC()
			m.lastEventIDs.Store(name, now)
			m.logger("Minecart: event poll (%s): +Inf/NaN row detected, advancing HWM to %s to skip corrupt data", name, now.Format(time.RFC3339))
			return nil
		}
		m.logger("Minecart: event poll error (%s): %v", name, err)
	...
	// Use hq store for minecart lookups (minecarts are hq-* prefixed)
	hqStore := stores["hq"]
	if hqStore == nil {
		m.logger("Minecart: hq store unavailable, skipping minecart lookups for %s events", name)
		return nil
	}
	...
	for _, e := range events {
		...
		m.logger("Minecart: close detected: %s (from %s)", issueID, name)
		resolver := minecart.NewStoreResolver(m.townRoot, stores)
		minecart.CheckMinecartsForIssue(m.ctx, hqStore, m.townRoot, issueID, "Minecart", m.logger, m.gtPath, m.isRigParked, resolver)
		minecart.FireCrossRigDepNotifications(m.ctx, issueID, m.townRoot, stores, m.logger)
	}
	return nil
}
...
// runStrandedScan is the periodic stranded minecart scan loop.
...
func (m *MinecartManager) runStrandedScan() {
	defer m.wg.Done()
...
// scan runs one stranded scan cycle: find stranded minecarts, feed or close each.
// Serialized by scanMu to prevent concurrent scans from spawning duplicate checks.
func (m *MinecartManager) scan() {
	m.scanMu.Lock()
	defer m.scanMu.Unlock()

	stranded, err := m.findStranded()
	if err != nil {
		m.logger("Minecart: stranded scan failed: %s", util.FirstLine(err.Error()))
		return
	}
	...
	for _, c := range stranded {
		...
		if c.ReadyCount > 0 {
			...
			// Empty minecart — but skip if it was just created (GH#2303).
			// The sling's bd dep add may not be visible in Dolt yet.
			if !c.CreatedAt.IsZero() && time.Since(c.CreatedAt) < minecartGracePeriod {
				m.logger("Minecart %s: empty but within grace period (created %s ago) — skipping", c.ID, time.Since(c.CreatedAt).Round(time.Second))
				continue
			}
			m.closeEmptyMinecart(c.ID)
		} else {
			// Tracked issues exist but none are ready. This could mean:
			// (a) all tracked issues are closed → minecart should auto-close
			// (b) issues are blocked/in-progress → needs agent review
			// Run minecart check to handle case (a); it's a no-op for (b).
			m.logger("Minecart %s: %d tracked issues, 0 ready — checking completion", c.ID, c.TrackedCount)
			m.checkMinecartCompletion(c.ID)
		}
	}
}
...
func (m *MinecartManager) findStranded() ([]strandedMinecartInfo, error) {
	...
	var stranded []strandedMinecartInfo
...
// feedFirstReady iterates through all ready issues in a stranded minecart and
...
// rig is parked, or the sling command fails. This ensures minecarts progress
// even when some issues target unavailable rigs.
func (m *MinecartManager) feedFirstReady(c strandedMinecartInfo) {
	...
	for _, issueID := range c.ReadyIssues {
		...
		if prefix == "" {
			m.logger("Minecart %s: no prefix for %s, skipping", c.ID, issueID)
			continue
		}

		rig := beads.GetRigNameForPrefix(m.townRoot, prefix)
		if rig == "" {
			m.logger("Minecart %s: no rig for %s (prefix %s), skipping", c.ID, issueID, prefix)
			continue
		}

		if m.isRigParked(rig) {
			m.logger("Minecart %s: rig %s is parked, skipping %s", c.ID, rig, issueID)
			continue
		}

		m.logger("Minecart %s: feeding %s to %s", c.ID, issueID, rig)
		...
		if err := cmd.Run(); err != nil {
			m.logger("Minecart %s: sling %s failed: %s", c.ID, issueID, util.FirstLine(stderr.String()))
			continue
		}
...
func (m *MinecartManager) checkMinecartCompletion(minecartID string) {
	cmd := exec.CommandContext(m.ctx, m.gtPath, "minecart", "check", minecartID)
	cmd.Dir = m.townRoot
	cmd.Env = bdMutationRoutingEnv(m.townRoot)
	util.SetProcessGroup(cmd)
	var stderr bytes.Buffer
	...
	if err := cmd.Run(); err != nil {
		m.logger("Minecart %s: completion check failed: %s", minecartID, util.FirstLine(stderr.String()))
	}
}
...
func (m *MinecartManager) closeEmptyMinecart(minecartID string) {
	...
	if err := cmd.Run(); err != nil {
		m.logger("Minecart %s: check failed: %s", minecartID, util.FirstLine(stderr.String()))
	}
}

// runStartupSweep runs one minecart check pass after a brief delay to catch
// minecarts that completed while the daemon was stopped or Dolt was unavailable.
...
func (m *MinecartManager) runStartupSweep() {
	...
	m.logger("Minecart: running startup sweep for stranded minecarts")
	m.scan()
}
