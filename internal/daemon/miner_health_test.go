...
// miner agent bead JSON. The descState parameter controls what appears in
...
// TestCheckMinerHealth_SkipsSpawning verifies that checkMinerHealth does NOT
...
func TestCheckMinerHealth_SkipsSpawning(t *testing.T) {
	...
	if strings.Contains(got, "CRASH DETECTED") {
		t.Errorf("spawning miner must not trigger CRASH DETECTED, got: %q", got)
	}
}

// TestCheckMinerHealth_DetectsCrashedMiner verifies that checkMinerHealth
// does detect a crash for a miner in agent_state=working with a dead session.
// This ensures the spawning guard in issue #1752 does not accidentally suppress
// legitimate crash detection for miners that were running normally.
func TestCheckMinerHealth_DetectsCrashedMiner(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mocks for tmux and bd")
	}
	binDir := t.TempDir()
	...
	got := logBuf.String()
	if !strings.Contains(got, "CRASH DETECTED") {
		t.Errorf("expected CRASH DETECTED for working miner with dead session, got: %q", got)
	}
}
...
// TestCheckMinerHealth_SkipsClosedHookBead verifies that checkMinerHealth
// does NOT fire CRASHED_MINER when the hook_bead is already closed.
// This is the regression test for the false-positive spam bug (issue hq-1o7):
// when a miner completes work normally, the hook_bead gets closed but the
// stale reference remains on the agent bead, causing repeated false alerts.
func TestCheckMinerHealth_SkipsClosedHookBead(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mocks for tmux and bd")
	}
	binDir := t.TempDir()
...
func TestCheckMinerHealth_SkipsDoneMiner(t *testing.T) {
	...
	if strings.Contains(got, "CRASH DETECTED") {
		t.Errorf("done miner with open hook_bead must not trigger CRASH DETECTED, got: %q", got)
	}
}
...
func TestCheckMinerHealth_SkipsNukedMiner(t *testing.T) {
	...
	if strings.Contains(got, "CRASH DETECTED") {
		t.Errorf("nuked miner must not trigger CRASH DETECTED, got: %q", got)
	}
}
...
// When hasWork is true, "list" returns an open work bead assigned to "myr/miners/mycat".
func writeFakeBDLookupFail(t *testing.T, dir string, hasWork bool) string {
	t.Helper()
	listOut := `[]`
	if hasWork {
		listOut = `[{"id":"wh-test-1","status":"open","assignee":"myr/miners/mycat"}]`
	}
	script := "#!/bin/sh\n" +
...
// TestReapIdleMiner_SkipsWhenBeadLookupFailsButHasWork verifies that reapIdleMiner
// does NOT kill a miner when the agent bead lookup fails but hasAssignedOpenWork
// confirms the miner has an open work bead assigned. This is the regression test
// for the working-bead-lookup-failed kill bug (GH#3342 followup).
func TestReapIdleMiner_SkipsWhenBeadLookupFailsButHasWork(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mocks for tmux and bd")
	}
	...
	d.reapIdleMiner("myr", "mycat", 15*time.Minute)

	if strings.Contains(logBuf.String(), "Reaping idle miner") {
		t.Errorf("must NOT reap miner with open assigned work when agent bead lookup fails, got: %q", logBuf.String())
	}
}

// TestReapIdleMiner_ReapsWhenBeadLookupFailsAndNoWork verifies that reapIdleMiner
// DOES kill a miner when the agent bead lookup fails, no work is assigned, and the
// agent process is not running. Ensures the hasAssignedOpenWork guard doesn't over-protect.
func TestReapIdleMiner_ReapsWhenBeadLookupFailsAndNoWork(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mocks for tmux and bd")
	}
	old := session.DefaultRegistry()
	...
	if !strings.Contains(logBuf.String(), "Reaping idle miner") {
		t.Errorf("expected idle miner with no work and failed bead lookup to be reaped, got: %q", logBuf.String())
	}
...
func TestReapIdleMiner_SkipsActiveAgent(t *testing.T) {
	...
	miner.TouchSessionHeartbeatWithState(townRoot, "myr-mycat", miner.HeartbeatWorking, "", "")
	...
	got := logBuf.String()
	if strings.Contains(got, "Reaping idle miner") {
		t.Errorf("must NOT reap miner with active agent process (GH#3342), got: %q", got)
	}
}

// TestReapIdleMiner_ReapsIdleNoHook verifies that reapIdleMiner DOES kill
// a miner whose hook_bead is missing AND whose agent process is NOT running
// (idle shell). This ensures the GH#3342 fix doesn't prevent legitimate reaping.
func TestReapIdleMiner_ReapsIdleNoHook(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("test uses Unix shell script mocks for tmux and bd")
	}
	// Register "myr" prefix so session name resolves to "myr-mycat"
	...
	got := logBuf.String()
	if !strings.Contains(got, "Reaping idle miner") {
		t.Errorf("expected idle miner with no agent to be reaped, got: %q", got)
	}
