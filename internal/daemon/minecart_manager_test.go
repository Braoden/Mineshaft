...
func TestScanStranded_ClosesEmptyMinecarts(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on Windows")
	}
...
func TestScanStranded_GracePeriodSkipsRecentMinecart(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on Windows")
	}

	// Minecart created 30 seconds ago — well within the 5-minute grace period.
	...
	if _, err := os.Stat(paths.checkLogPath); err == nil {
		...
		t.Errorf("minecart check was called for recent minecart (grace period should protect): %s", data)
	}
...
func TestScanStranded_GracePeriodAllowsOldMinecart(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on Windows")
	}

	// Minecart created 10 minutes ago — past the 5-minute grace period.
...
func TestScanStranded_NoStrandedMinecarts(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on Windows")
	}
	...
	// Negative: minecart check must not have been called
	if _, err := os.Stat(paths.checkLogPath); err == nil {
		data, _ := os.ReadFile(paths.checkLogPath)
		t.Errorf("minecart check was called unexpectedly: %s", data)
	}
	...
	for _, s := range logged {
		if strings.Contains(s, "feeding") || strings.Contains(s, "sling") || strings.Contains(s, "auto-closing") {
			t.Errorf("unexpected minecart activity in logs: %s", s)
		}
	}
}
...
func TestMinecartManager_DoubleStop_Idempotent(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on Windows")
	}
	binDir := t.TempDir()
...
func TestStrandedMinecartInfo_JSONParsing(t *testing.T) {
	...
	if len(result) != 1 {
		t.Fatalf("expected 1 minecart, got %d", len(result))
	}
	c := result[0]
...
func TestFeedFirstReady_IteratesPastDispatchFailure(t *testing.T) {
	...
	// Minecart has 3 ready issues. First sling fails, second succeeds.
	// Verifies feedFirstReady iterates past dispatch failure within a single minecart.
	binDir := t.TempDir()
	townRoot := t.TempDir()
	if err := os.MkdirAll(filepath.Join(townRoot, ".beads"), 0755); err != nil {
		t.Fatalf("mkdir .beads: %v", err)
...
func TestScan_ContextCancelled_MidIteration(t *testing.T) {
	...
	minecarts := make([]minecart, 5)
	for i := range minecarts {
...
`
	...
	// scan() must exit cleanly within a bounded time (not hang on all 5 minecarts)
	select {
	case <-done:
		// Clean exit -- success
	case <-time.After(5 * time.Second):
	...
	// Verify it did NOT process all 5 minecarts (cancellation stopped iteration)
	logMu.Lock()
	defer logMu.Unlock()
	feedCount := 0
	for _, s := range logged {
	...
	if feedCount >= 5 {
		t.Errorf("expected cancellation to stop iteration before all 5 minecarts, but all were fed")
	}
}
...
func TestScanStranded_MixedReadyAndEmpty(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("read check log: %v (minecart check was never called)", err)
	}
	checkContent := string(checkData)
	if !strings.Contains(checkContent, "hq-empty1") {
		t.Errorf("expected minecart check for hq-empty1 (empty minecart), got: %q", checkContent)
	}
	if !strings.Contains(checkContent, "hq-empty2") {
		t.Errorf("expected minecart check for hq-empty2 (empty minecart), got: %q", checkContent)
	}

	// Negative: ready minecarts should NOT appear in check log
	if strings.Contains(checkContent, "hq-ready1") {
		t.Errorf("ready minecart hq-ready1 should not appear in check log: %q", checkContent)
	}
	if strings.Contains(checkContent, "hq-ready2") {
		t.Errorf("ready minecart hq-ready2 should not appear in check log: %q", checkContent)
	}

	// Negative: empty minecarts should NOT appear in sling log
	if strings.Contains(slingContent, "hq-empty1") || strings.Contains(slingContent, "hq-empty2") {
		t.Errorf("empty minecarts should not appear in sling log: %q", slingContent)
	}
}
...
func TestPollStore_NilHqStore_LogsWarningAndSkips(t *testing.T) {
	...
	// The nil hq guard should log a warning and skip minecart lookups.
	rigStore, rigCleanup := setupTestStore(t)
	defer rigCleanup()
...
// are serialized by scanMu (no duplicate minecart checks).
