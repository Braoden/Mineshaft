...
func TestGetIssueDetailsBatch_ReturnsStructuredErrorOnCommandFailure(t *testing.T) {
	...
	_, err := f.getIssueDetailsBatch([]string{"ms-1", "ms-2"})
	if err == nil {
...
func TestGetIssueDetailsBatch_ReturnsStructuredErrorOnInvalidJSON(t *testing.T) {
	...
	f := &LiveMinecartFetcher{cmdTimeout: 100 * time.Millisecond}
	_, err := f.getIssueDetailsBatch([]string{"ms-9"})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestGetIssueDetailsBatch_ParsesIssueDetails(t *testing.T) {
	...
	fetcherRunCmd = func(_ time.Duration, _ string, _ ...string) (*bytes.Buffer, error) {
		return bytes.NewBufferString(`[
			{"id":"ms-1","title":"One","status":"open","assignee":"rig/miners/a","updated_at":"2026-02-01T12:00:00Z"},
			{"id":"ms-2","title":"Two","status":"closed","assignee":"","updated_at":"2026-02-01T12:01:00Z"}
		]`), nil
	}

	f := &LiveMinecartFetcher{cmdTimeout: 100 * time.Millisecond}
	details, err := f.getIssueDetailsBatch([]string{"ms-1", "ms-2"})
	if err != nil {
	...
	if details["ms-1"] == nil || details["ms-1"].Title != "One" {
		t.Fatalf("unexpected parsed details for ms-1: %#v", details["ms-1"])
	}
	if details["ms-2"] == nil || details["ms-2"].Status != "closed" {
		t.Fatalf("unexpected parsed details for ms-2: %#v", details["ms-2"])
	}
}
