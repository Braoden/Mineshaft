...
func TestGetPauseFile(t *testing.T) {
	...
	expected := filepath.Join(townRoot, ".runtime", "supervisor", "paused.json")
...
func TestPause_FileContent(t *testing.T) {
	...
	if err := Pause(tmpDir, "maintenance window", "overseer"); err != nil {
		t.Fatalf("Pause() error = %v", err)
	}
	...
	if state.PausedBy != "overseer" {
		t.Errorf("state.PausedBy = %q, want %q", state.PausedBy, "overseer")
	}
	if state.PausedAt.IsZero() {
		t.Error("state.PausedAt should not be zero")
	}
