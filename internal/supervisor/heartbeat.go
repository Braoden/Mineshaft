...
// Configurable via operational.supervisor.heartbeat_stale_threshold and
// operational.supervisor.heartbeat_very_stale_threshold in settings/config.json.
const (
	...
	// very stale, meaning the Supervisor should be poked or restarted.
...
// Heartbeat represents the Supervisor's heartbeat file contents.
// Written by the Supervisor on each wake cycle.
...
type Heartbeat struct {
	...
	// LastAction describes what the Supervisor did in this cycle.
...
// HeartbeatFile returns the path to the Supervisor heartbeat file.
func HeartbeatFile(townRoot string) string {
	return filepath.Join(townRoot, "supervisor", "heartbeat.json")
}

// WriteHeartbeat writes a new heartbeat to disk.
// Called by the Supervisor at the start of each wake cycle.
func WriteHeartbeat(townRoot string, hb *Heartbeat) error {
	...
	// Also touch .supervisor-heartbeat for backward compatibility with shell scripts
	...
	legacyFile := filepath.Join(filepath.Dir(hbFile), ".supervisor-heartbeat")
	_ = os.WriteFile(legacyFile, []byte(""), 0644) //nolint:gosec // G306: world-readable liveness file is intentional

	return nil
}

// ReadHeartbeat reads the Supervisor heartbeat from disk.
...
// A fresh heartbeat means the Supervisor is actively working or recently finished.
...
// A stale heartbeat may indicate the Supervisor is doing a long operation.
...
// A very stale heartbeat means the Supervisor should be poked.
