...
// Configurable via operational.supervisor in settings/config.json.
const (
...
func LoadStuckConfig(townRoot string) *StuckConfig {
	...
	supervisorCfg := opCfg.GetSupervisorConfig()
	return &StuckConfig{
		PingTimeout:         supervisorCfg.PingTimeoutD(),
		ConsecutiveFailures: supervisorCfg.ConsecutiveFailuresV(),
		Cooldown:            supervisorCfg.CooldownD(),
	}
}

// AgentHealthState tracks the health check state for a single agent.
type AgentHealthState struct {
	// AgentID is the identifier (e.g., "excavation/miners/max" or "supervisor")
...
func HealthCheckStateFile(townRoot string) string {
	return filepath.Join(townRoot, "supervisor", "health-check-state.json")
}
