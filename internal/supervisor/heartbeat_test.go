...
func TestHeartbeatFile(t *testing.T) {
	...
	expected := filepath.Join(townRoot, "supervisor", "heartbeat.json")
...
func TestWriteHeartbeat_CreatesDirectory(t *testing.T) {
	...
	// Ensure supervisor directory doesn't exist
	supervisorDir := filepath.Join(tmpDir, "supervisor")
	if _, err := os.Stat(supervisorDir); !os.IsNotExist(err) {
		t.Fatal("supervisor directory should not exist initially")
	}
	...
	if _, err := os.Stat(supervisorDir); err != nil {
		t.Errorf("supervisor directory should exist: %v", err)
	}
}
...
func TestWriteHeartbeat_TouchesLegacyFile(t *testing.T) {
	...
	// Legacy .supervisor-heartbeat should also exist so shell scripts (stuck-agent-dog)
	// that check this file's mtime get accurate data.
	legacyFile := filepath.Join(tmpDir, "supervisor", ".supervisor-heartbeat")
	info, err := os.Stat(legacyFile)
	if err != nil {
		t.Errorf(".supervisor-heartbeat not created: %v", err)
		return
	}
	if time.Since(info.ModTime()) > time.Minute {
		t.Error(".supervisor-heartbeat mtime should be recent")
	}
}
