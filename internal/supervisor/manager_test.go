...
func TestManager_supervisorDir(t *testing.T) {
	m := NewManager("/tmp/test-town")
	expected := filepath.Join("/tmp/test-town", "supervisor")
	if m.supervisorDir() != expected {
		t.Errorf("supervisorDir() = %q, want %q", m.supervisorDir(), expected)
	}
}
