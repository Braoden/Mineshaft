...
// Configurable via operational.supervisor.max_redispatches and
// operational.supervisor.redispatch_cooldown in settings/config.json.
const (
	// DefaultMaxRedispatches is the number of times a bead can be re-dispatched
	// before escalating to Overseer instead of re-slinging.
	DefaultMaxRedispatches = 3

	// DefaultRedispatchCooldown is the minimum time between re-dispatches of
	// the same bead. Prevents thrashing when a bead keeps killing miners.
	DefaultRedispatchCooldown = 5 * time.Minute
)

// RedispatchState tracks re-dispatch attempts for recovered beads.
// Persisted to supervisor/redispatch-state.json.
...
type BeadRedispatchState struct {
	...
	// Escalated is true if this bead has been escalated to Overseer.
...
func RedispatchStateFile(townRoot string) string {
	return filepath.Join(townRoot, "supervisor", "redispatch-state.json")
}
...
// RecordEscalation records that the bead was escalated to Overseer.
...
func Redispatch(townRoot, beadID, sourceRig string, maxAttempts int, cooldown time.Duration) *RedispatchResult {
	...
	if beadState.Escalated {
		...
		result.Message = fmt.Sprintf("bead already escalated to Overseer at %s", beadState.EscalatedAt.Format(time.RFC3339))
		return result
	}
	...
	if beadState.ShouldEscalate(maxAttempts) {
		...
		err := escalateToOverseer(townRoot, beadID, beadState)
		if err != nil {
			result.Error = fmt.Errorf("escalating to overseer: %w", err)
			result.Message = fmt.Sprintf("failed to escalate after %d attempts: %v", beadState.AttemptCount, err)
		} else {
			beadState.RecordEscalation()
			result.Message = fmt.Sprintf("escalated to Overseer after %d failed re-dispatches", beadState.AttemptCount)
		}
...
// escalateToOverseer sends an escalation mail to the Overseer about a repeatedly-failing bead.
func escalateToOverseer(townRoot, beadID string, beadState *BeadRedispatchState) error {
...
This bead may have a systemic issue (e.g., causes miner crashes).
...
// Looks for "Miner: <rig>/<name>" line.
func ParseRecoveredBeadBody(body string) (rig string) {
	for _, line := range strings.Split(body, "\n") {
		line = strings.TrimSpace(line)
		if strings.HasPrefix(line, "Miner:") {
			minerAddr := strings.TrimSpace(strings.TrimPrefix(line, "Miner:"))
			parts := strings.SplitN(minerAddr, "/", 2)
			if len(parts) >= 1 {
				return parts[0]
			}
		}
