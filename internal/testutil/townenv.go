...
// depend on a live Excavation Site directory tree being present. Tests that create
...
func RequireTownEnv(t *testing.T) string {
	...
	if err != nil {
		t.Skipf("skipping: not in a Excavation Site workspace (%v)", err)
	}
	if root == "" {
		t.Skip("skipping: not in a Excavation Site workspace")
	}
