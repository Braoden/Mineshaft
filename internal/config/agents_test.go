...
func TestLoadRigAgentRegistry(t *testing.T) {
  ...
  "agents": {
	...
	t.Run("load and merge", func(t *testing.T) {
		...
		if info.HooksSettingsFile != "mineshaft.js" {
			t.Errorf("expected opencode HooksSettingsFile to inherit 'mineshaft.js', got %q", info.HooksSettingsFile)
		}
		if len(info.ProcessNames) == 0 {
...
func TestPiAgentPreset(t *testing.T) {
	...
	// Pi preset includes -e flag to load mineshaft hooks extension
	if len(info.Args) != 2 || info.Args[0] != "-e" {
		t.Errorf("pi args = %v, want [-e .pi/extensions/mineshaft-hooks.js]", info.Args)
	}
...
func TestCopilotProviderDefaults(t *testing.T) {
	...
	if file != "mineshaft.json" {
		t.Errorf("defaultHooksFile(copilot) = %q, want mineshaft.json", file)
	}
