...
// - All SessionStart hooks with `ms prime` include the `--hook` flag
...
// `ms prime` include the `--hook` flag. Without this flag, sessions won't
...
func TestSessionStartHooksHaveHookFlag(t *testing.T) {
	...
	for _, path := range settingsFiles {
		...
		for _, entry := range sessionStartHooks {
			for _, hook := range entry.Hooks {
				cmd := hook.Command
				// Check if command contains "ms prime" but not "--hook"
				if strings.Contains(cmd, "ms prime") && !strings.Contains(cmd, "--hook") {
	...
	if len(failures) > 0 {
		t.Errorf("SessionStart hooks missing --hook flag in ms prime command:\n  %s\n\n"+
			"The --hook flag is required for seance to discover predecessor sessions.\n"+
			"Fix by changing 'ms prime' to 'ms prime --hook' in these files.",
...
// settings files in the town, any SessionStart hook containing "ms prime"
...
func TestAllAgentSessionStartHooksHaveHookFlag(t *testing.T) {
	...
	for _, pattern := range knownAgentPatterns {
		...
		err := filepath.Walk(townRoot, func(path string, info os.FileInfo, err error) error {
			...
			for _, entry := range sessionStartHooks {
				for _, hook := range entry.Hooks {
					if strings.Contains(hook.Command, "ms prime") && !strings.Contains(hook.Command, "--hook") {
						relPath, _ := filepath.Rel(townRoot, path)
						failures = append(failures, relPath)
					}
				}
