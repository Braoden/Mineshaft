...
func TestNewMailboxBeads(t *testing.T) {
	m := NewMailboxBeads("mineshaft/Toast", "/work/dir")
	if m.identity != "mineshaft/Toast" {
		t.Errorf("identity = %q, want %q", m.identity, "mineshaft/Toast")
	}
	if m.legacy {
...
func TestMailboxIdentityAndPath(t *testing.T) {
	...
	// Beads mailbox
	beads := NewMailboxBeads("mineshaft/Toast", "/work/dir")
	if beads.Identity() != "mineshaft/Toast" {
		t.Errorf("Beads mailbox identity = %q, want mineshaft/Toast", beads.Identity())
	}
	if beads.Path() != "" {
...
func TestNewMailboxWithBeadsDir(t *testing.T) {
	m := NewMailboxWithBeadsDir("mineshaft/Toast", "/work/dir", "/custom/.beads")
	if m.identity != "mineshaft/Toast" {
		t.Errorf("identity = %q, want 'mineshaft/Toast'", m.identity)
	}
...
`
	...
	m := NewMailboxWithBeadsDir("mineshaft/synth", t.TempDir(), beadsDir)
